...
Opacity is an *attribute* of a record type.  Attributes are an experimental feature, particularly their syntax, and may change radically in the future.  At present, the attributes of a record type are those relating to opacity, described below, and ``variables``, which specifies default variable names for that type (see :ref:`Default names`); datatypes and codatatypes can also have a ``variables`` attribute.  The current syntax for defining a record type with an attribute is ``sig #(ATTR) ( … )``, and multiple attributes can be specified with multiple ``#(ATTR)`` groups.  Currently attributes can only be set when a record type is defined; in the future it may be possible to alter them after the fact.  Opacity attributes do *not* affect convertibility of terms; η-conversion is always valid internally.  Opacity attributes only affect how terms are *displayed* to the user.  (If you want a record-like type without η-conversion, use a non-recursive codatatype; see :ref:`Codatatypes and comatching`.)
