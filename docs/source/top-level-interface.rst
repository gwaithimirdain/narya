...
A canonical type displayed at a positive dimension carries that dimension as a superscript on its introducing keyword, as in ``data⁽ᵉ⁾ [ … ]``, ``codata⁽ᵉ⁾ [ … ]``, and ``sig⁽ᵉ⁾ ( … )``.  The dimension shown is the *evaluation* dimension, the one it was degenerated by: an intrinsically higher-dimensional (Gel-like) codatatype that has not been degenerated, such as ``Gel A B R``, still displays as an undecorated ``sig``.  These superscripts are display-only and are not accepted by the parser, since the only way to write such a type is to degenerate an undegenerated one.
