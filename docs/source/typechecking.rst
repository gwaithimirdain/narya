...
The ascription notation has tightness −ω, and is right-associative, so that ``M : N : P`` means ``M : (N : P)`` (although this would be a very rare thing to need to write).  Since ``↦`` and ``let`` have tightness −ω+1, they parse inside it, so that for instance ``x ↦ M : A → B`` means ``(x ↦ M) : (A → B)``.
