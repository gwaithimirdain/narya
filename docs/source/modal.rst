...
There is not yet a syntax for printing and referring to modal cells.  Thus, although internally Narya supports arbitrary (cofibrant) 2-categories as mode theories, at present it is only possible to actually *use* mode theories that are *locally posetal*, meaning that any two parallel 2-cells are equal.  Each mode theory supplies an algorithm for finding such 2-cells automatically whenever needed.
