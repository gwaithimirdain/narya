...
.. csv-table:: Mode theories
   ...
   "``-functor``", "``DomType``, ``CodType``", "``○ : DomType → CodType``", "--"
   "``-composed-functors``", "``AType``, ``BType``, ``CType``", "| ``○ : AType → BType``,
   | ``▱ : BType → CType``", "--"
   "| ``-coreflector``,
...
* ``-spatial`` type theory was so-called in the paper `Brouwer's fixed-point theorem in real-cohesive homotopy type theory <https://www.cambridge.org/core/journals/mathematical-structures-in-computer-science/article/abs/brouwers-fixedpoint-theorem-in-realcohesive-homotopy-type-theory/8270C2EAC4EE5D5CDBA17EEB3FF6B19E>`_ because its intended models were toposes of spaces, with ``♭`` assigning the discrete topology and ``♯`` the codiscrete one.
...
* ``-adjunction`` is the "walking adjunction": it is freely generated by a single adjunction ``△ ⊣ □``, whose unit ``1 ⇒ □△`` and counit ``△□ ⇒ 1`` satisfy the triangle identities but nothing further.  In particular, no composite modality reduces, so the modalities are exactly the alternating words in ``△`` and ``□``.  It is not locally posetal (and neither is ``-gwpt``): parallel 2-cells can be distinct, such as the two whiskerings ``□△ ⇒ □△□△`` of the unit.  (Two 2-cells are equal exactly when they induce the same "pairing" of the generators occurring in their domain and codomain.)  Since Narya does not yet have syntax for specifying an explicit 2-cell, any use of a variable that requires a non-unique 2-cell as a key is an error.
...
* ``-gwpt`` is short for "geometrically well-pointed topos".  It is generated by two adjunctions ``△ ⊣ □`` and ``◇ ⊣ ∇``, where the counit ``◇∇ ⇒ 1`` of the second is invertible (that is, ``∇`` is fully faithful) but its unit ``1 ⇒ ∇◇`` is not, and neither the unit nor the counit of ``△ ⊣ □`` is; in addition ``◇△ ≅ 1``, and hence (passing to right adjoints) also ``□∇ ≅ 1``.  These isomorphisms induce 2-cells ``□ ⇒ ◇`` and ``△ ⇒ ∇``.  Every modality is isomorphic to a unique normal form: an alternating word in ``△`` and ``□``, possibly starting (at the right, in applicative order) with a single ``◇`` and/or ending (at the left) with a single ``∇``.  Like ``-adjunction``, this theory is not locally posetal, and parallel 2-cells are compared by their induced pairings; any use of a variable that requires a non-unique 2-cell as a key is an error.
