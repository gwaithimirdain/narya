...
Note also that ``A`` itself is ``□``-annotated and lives at the mode ``Type``, the domain of ``□``.  In general, the type of a modal field is typechecked in a context locked by the *right adjoint*, which in the above case is ``□`` so that we can use the ``□``-annotated variable ``A``.  More precisely, that context is the ambient one locked by the right adjoint and *then* extended by the self-variable, which is annotated by the left adjoint; thus in the above example the context of ``A``, the type of ``.unbox``, is
...
The self-variable can therefore be used in the field's type (and in the types of later fields) whenever there is a key from the left adjoint to the composite of the locks to its right, just as for any other variable.  (This is equivalent to the more naive-looking alternative of extending by an identity-annotated self-variable and then locking by the right adjoint, since the required keys ``f ⇒ ν`` and ``1 ⇒ g ν`` are interderivable using the unit and counit of the adjunction; but the above is the presentation used by Multimodal Adjoint Type Theory.)

A modal :ref:`higher field <Higher coinductive types>` works the same way, except that the context is first degenerated by the field's intrinsic dimension, so that the self-variable is a cube of that dimension.  For instance, in the ``-spatial`` mode theory,

.. code-block:: none

   def √♭ (B :♯| Type) : Type ≔ codata [ (x :♭| _) .root.e : ? ]
