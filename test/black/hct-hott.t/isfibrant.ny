...
| x .trr.e : A.0 → A.1
| x .trl.e : A.1 → A.0
| x .liftr.e : (a₀ : A.0) → A.2 a₀ (x.2 .trr a₀)
| x .liftl.e : (a₁ : A.1) → A.2 (x.2 .trl a₁) a₁
...
def Id𝕗 (A : Fib) (x y : A .t) : Fib ≔ (Id (A .t) x y, refl A .f .id x y)
...
def concat (A : Fib) (x y z : A .t) (p : Id (A .t) x y) (q : Id (A .t) y z)
  : Id (A .t) x z
  ≔ refl (Id𝕗 A x) y z q .f .trr p

def inverse (A : Fib) (x y : A .t) (p : Id (A .t) x y) : Id (A .t) y x
  ≔ refl ((z ↦ Id𝕗 A z x) : A .t → Fib) x y p .f .trr (refl x)
...
def transport2 (A : Type) (B : A → Fib) (x y : A) (p q : Id A x y)
  ...
  ≔ B⁽ᵉᵉ⁾ x x (refl x) y y (refl y) p q r
      .f
      .id.2 b (transport A B x y p b) (refl B x y p .f .liftr b) b
        (transport A B x y q b) (refl B x y q .f .liftr b)
      .trr (refl b)
...
def concat_p1 (A : Fib) (x y : A .t) (p : Id (A .t) x y)
  ...
  ≔ refl ((q ↦ Id𝕗2 A x y q p) : Id (A .t) x y → Fib) p
        (concat A x y y p (refl y)) (refl (Id𝕗 A x) y y (refl y) .f .liftr p)
      .f
      .trr (refl p)
...
def J (A : Fib) (a : A .t) (P : (y : A .t) → Id (A .t) a y → Fib)
  ...
  let q ≔ sq .trr (refl a) in
  let s ≔ sq .liftr (refl a) in
  refl P a b q (refl a) p (sym s) .f .trr pa
...
def Sq𝕗 (A : Fib) (x00 x01 : A .t) (x02 : Id (A .t) x00 x01) (x10 x11 : A .t)
  ...
  A⁽ᵉᵉ⁾ .f .id.1 x00 x01 x02 x10 x11 x12 .id x20 x21)
...
def concat_1p (A : Fib) (x y : A .t) (p : Id (A .t) x y)
  ...
  ≔ refl (Id𝕗2 A x) x y p (refl x) (concat A x x y (refl x) p)
        (refl (Id𝕗 A x) x y p .f .liftr (refl x)) (refl x) p (coconn A x y p)
      .f
      .trr (refl (refl x))
...
def Jβ (A : Fib) (a : A .t) (P : (y : A .t) → Id (A .t) a y → Fib)
  ...
  let cube
    ≔ refl (Sq𝕗 A) a a (refl a) a a (refl a) (refl a) (refl a) a⁽ᵉᵉ⁾ a a
        ...
        (refl a) a⁽ᵉᵉ⁾ .f in
  let t ≔ cube .trr a⁽ᵉᵉ⁾ in
  let c ≔ cube .liftr a⁽ᵉᵉ⁾ in
  P⁽ᵉᵉ⁾ a a (refl a) a a (refl a) (refl a) q (sym t) (refl a) (refl a) a⁽ᵉᵉ⁾
    ...
    .id.2 pa pa (refl pa) pa (J A a P pa a (refl a))
      (refl P a a q (refl a) (refl a) (sym s) .f .liftr pa)
    .trr (refl pa)
