...
side), so an implicit key cannot be inserted.
  ...
  $ narya -adjunction adjunction.ny -e "def amb (A : Disc) (x : □△ | A) : T2 A ≔ t2. x"
   ￫ error[E1705]
   ￭ command-line exec string
   1 | def amb (A : Disc) (x : □△ | A) : T2 A ≔ t2. x
...
either side).
  ...
  $ narya -adjunction adjunction.ny -e "def amb2 (A : △□ | Type) (x : △□△□ | A) : S A ≔ s. x"
   ￫ error[E1705]
   ￭ command-line exec string
   1 | def amb2 (A : △□ | Type) (x : △□△□ | A) : S A ≔ s. x
     ^ use of △□△□ variable behind △□ lock requires a key
