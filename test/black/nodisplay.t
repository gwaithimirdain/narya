...
A *higher* field still does not.  Its instances are read back in a context degenerated by their remaining dimensions, and the boundary there is rebuilt by that degeneration rather than read off the type, so it is not specialized and the instance body is read back at a type whose instantiation arguments are not the constructors it expects.
