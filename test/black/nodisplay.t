...
A branch body that is itself an *anonymous* codatatype or record -- one written inline rather than named, so that it evaluates to a canonical value rather than to a neutral -- is displayed as a declaration rather than as a spine, which at a positive dimension means computing its field types.  Those project the field from the self variable at each proper face of the self cube, and those faces come from the type we are reading back at, which is the *match's*: they are the match at the faces of the environment, and a stuck match exposes no fields.  Specializing them at this branch's constructor makes them the branch's own faces instead, so a lower field displays, and so does a record.
  ...
  $ narya -e 'def N : Type ≔ data [ zero. | suc. (_ : N) ]' -e 'def Bool : Type ≔ data [ true. | false. ]' -e 'axiom b0 : Bool' -e 'axiom b1 : Bool' -e 'axiom b2 : Id Bool b0 b1' -e 'def G : Bool → Type ≔ [ true. ↦ sig ( a : N ) | false. ↦ N ]' -e 'about (refl G b2)'
  match b2 [
  | false. ⤇ N⁽ᵉ⁾
  | true. ⤇ sig⁽ᵉ⁾ (
      𝑥 .a : N⁽ᵉ⁾ (𝑥.0 .a) (𝑥.1 .a) )]
...
A higher field displays too.  Its instances are read back in a context degenerated by their remaining dimensions, which reads the self back and so reads back the constructor it is specialized at; a higher-dimensional constructor is read back at the datatype instantiated at the constructors below it, which is the type constr_norm_cube gives each face of the constructor cube, rather than at the discriminee's own boundary.

  $ narya -v -e 'def N : Type ≔ data [ zero. | suc. (_ : N) ]' -e 'def Bool : Type ≔ data [ true. | false. ]' -e 'axiom b0 : Bool' -e 'axiom b1 : Bool' -e 'axiom b2 : Id Bool b0 b1' -e 'def F : Bool → Type ≔ [ true. ↦ codata [ x .root.e : N ] | false. ↦ N ]' -e 'about (refl F b2)'
   ￫ info[I0000]
   ￮ constant N defined
...
And at dimension zero there is no boundary to instantiate at, so nothing needs specializing.
...
Specializing the boundary is what makes this work, and it cannot be replaced by asking whether every face of the self cube is already a codatatype.  An intrinsically higher, Gel-like codatatype has for its boundary the endpoint types of the path that it is, which are arbitrary types that its field types take as arguments rather than project from.
...
Reading a branch back at an unrefined type can also fail inside a constructor, when the type is a degenerate datatype whose instantiation arguments are not the corresponding constructors.  The same match with a motive displays; it is the unrefined type, not the degenerated environment, that fails here.
...
A convoy -- a match the case tree applies to arguments -- is degraded the same way, and by the same cause.  Its "return" clause needs the matches at the faces of the environment, which for a convoy are got by unapplying the faces of the type it carries: those are the *convoys* there, and an unapplication discards what the case tree applied them to.  Where a face has reduced there is no match to unapply, and we show the convoy without a "return" clause.  This is the same "def g" as above made into a convoy, and it degrades for the same reason: the discriminee is a path between two constructors, so the match at each endpoint reduces.

  $ narya -e 'def N : Type ≔ data [ zero. | suc. (_ : N) ]' -e 'def Bool : Type ≔ data [ true. | false. ]' -e 'def T : Bool → Type ≔ [ true. ↦ N | false. ↦ Bool ]' -e 'def g (b : Bool) (n : N) : T b ≔ (match b return x ↦ N → T x [ true. ↦ m ↦ zero. | false. ↦ m ↦ true. ]) n' -e 'axiom p : Id Bool true. false.' -e 'about (refl g p)'
  n ⤇ match p [ false. ⤇ m ⤇ true. | true. ⤇ m ⤇ refl 0 ] n.2
    : {n₀ : N} {n₁ : N} (n₂ : N⁽ᵉ⁾ n₀ n₁) →⁽ᵉ⁾ Id T p 0 true.
