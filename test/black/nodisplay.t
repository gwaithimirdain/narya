...
A *higher* field still does not.  Its instances are read back in a context degenerated by their remaining dimensions, and the boundary there is rebuilt by that degeneration rather than read off the type, so it is not specialized and the instance body is read back at a type whose instantiation arguments are not the constructors it expects.  With an explicit motive that degeneration also meets the branch's stored type, which a specialization carries rather than computes, and which evaluated at the higher dimension is not instantiated at the faces the degeneration adds; that is caught and reported too, and the branch falls back either way.
...
So does an anonymous *datatype*, under an implicit match: readback of a degenerate datatype evaluates each constructor's function-type in a faced environment instead of projecting from boundary self variables, so it never needs the boundary to be a datatype at all.  (Under an explicit motive it falls back, at the unrefined branch type, like the constructors in III.)
