...
A branch that reaches a higher codata field used to be the one failure any match could suffer.  Raising the self into the context degenerated by a non-projectable instance's remaining dimensions reads it back and re-evaluates it, which it survives only if its spine really does evaluate to the comatch; a branch whose discriminee could not be refined left the spine a stuck match, so projecting from it computed nothing.  It was worse for a match with an explicit motive than for an implicit one, since refinement is skipped whenever there is a motive to apply instead.

The self now carries a specialization, a display-only spine entry that reduces the match at the head end as if its discriminee were this branch's constructor, and that survives the cycle because reading it back emits a term that evaluates to the same specialization again.  So all three display: an implicit match on a variable discriminee, the same match with a motive, and an implicit one whose discriminee is not a variable at all.

  $ narya -v -e 'def N : Type ≔ data [ zero. | suc. (_ : N) ]' -e 'def √N : Type ≔ codata [ x .root.e : N ]' -e 'axiom ax : N' -e 'about (let u : N → √N ≔ [ zero. ↦ [ .root.e ↦ zero. ] | suc. n ↦ [ .root.e ↦ suc. zero. ] ] in u)' -e 'about (let v : N → √N ≔ n ↦ match n return _ ↦ √N [ zero. ↦ [ .root.e ↦ zero. ] | suc. k ↦ [ .root.e ↦ suc. zero. ] ] in v)' -e 'about (let w : N → √N ≔ [ zero. ↦ [ .root.e ↦ zero. ] | suc. n ↦ [ .root.e ↦ suc. zero. ] ] in w ax)'
   ￫ info[I0000]
   ￮ constant N defined
  ...
  n ↦
  match n
  return 𝑥 ↦ √N [ suc. k ↦ [ .root.e ↦ 1 ] | zero. ↦ [ .root.e ↦ 0 ] ]
    : N → √N
