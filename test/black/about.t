...
An indexed datatype that has been applied to its parameters but not yet to all of its indices is a canonical type whose *type* is a pi-type rather than a universe, the remaining indices being still to come.  It displays as the datatype declaration, with the indices supplied so far given as an application afterwards, just as for a fully applied one.

  $ narya -e 'def N : Type ≔ data [ zero. | suc. (_ : N) ]' -e 'def Vec : Type → N → Type ≔ A ↦ data [ nil. : Vec A zero. | cons. : (n : N) → A → Vec A n → Vec A (suc. n) ]' -e 'axiom X : Type' -e 'about (Vec X)'
  data [
  | nil. : Vec X 0
  | cons. (n : N) (𝑥 : X) (𝑦 : Vec X n) : Vec X (suc. n) ]
    : N → Type
...
The *other* boundary, the one motive_branch_ty needs, comes from the same place: the type at which to read back a branch body is the motive instantiated at the faces of that body, and a branch body is a case tree, whose value at a face may be a case tree too rather than a term.  When it is, we take the match at that face -- again from the instantiation arguments of the type -- and specialize it at this branch's constructor there, which denotes the same thing and is a neutral, hence a term.  So the bodies here, which are lambdas, no longer stop it.
...
The datatype in a branch body can be an indexed one that the branch does not apply to all of its indices, in which case the type at which the body is read back is a pi-type rather than a universe.
...
A deep match is compiled into a nest of matches, of which the user wrote only the outermost.  When the user gave that one a motive, the nested ones get motives too, so that the whole nest displays as the user's one match did.  A nested match with nothing after its discriminee gets the goal abstracted over it -- here "𝑥 ↦ T (w. 𝑥)", which is what a hand-written convoy on that variable would have to say.  Without it this fell back on its application spine, since a branch body that is a bare constructor does not synthesize and an implicit match has no motive to read it back at.

  $ narya -e 'def N : Type ≔ data [ zero. | suc. (_ : N) ]' -e 'def Bool : Type ≔ data [ true. | false. ]' -e 'def W : Type ≔ data [ w. (_ : Bool) ]' -e 'def T : W → Type ≔ [ w. true. ↦ N → N | w. false. ↦ N ]' -e 'axiom b : Bool' -e 'about (let f : (x : W) → T x ≔ x ↦ match x return y ↦ T y [ w. true. ↦ n ↦ n | w. false. ↦ zero. ] in f (w. b))'
  match b
  return 𝑥 ↦ T (w. 𝑥) [ false. ↦ 0 | true. ↦ n ↦ n ]
    : T (w. b)
