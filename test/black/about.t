...
A comatch against a codatatype with a higher field is read back by reading back each field instance's body: non-projectable higher instances (like the declaration ".root.e") in a context degenerated by the partial bijection's remaining dimensions, evaluating the comatch's stored field term there.  A *degenerate* higher comatch (e.g. "refl r" of one) isn't read back (that's out of scope, and a degeneracy can't wrap a comatch term), so it shows as its application spine.
