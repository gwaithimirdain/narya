...
def cong (X Y : Type) (f : X → Y) (x y : X) (e : eq X x y) : eq Y (f x) (f y)
  ≔ match e [ rfl. ↦ rfl. ]
def trans (X : Type) (x y z : X) (p : eq X x y) (q : eq X y z) : eq X x z
  ≔ match q [ rfl. ↦ p ]
def rev (X : Type) (x y : X) (p : eq X x y) : eq X y x ≔ match p [
| rfl. ↦ rfl.]
def tr (X : Type) (P : X → Type) (x y : X) (p : eq X x y) : (P x) → (P y)
  ≔ match p [ rfl. ↦ u ↦ u ]
...
def uniq (A : Type) (zA : A) (sA : A → A) (f : cnat → A)
  (zf : eq A (f czero) zA) (sf : (n : cnat) → eq A (f (csuc n)) (sA (f n)))
  (n : cnat)
...
def cnat_ind (P : cnat → Type) (p0 : (P czero))
...
def cnat_add : cnat → (cnat → cnat)
  ≔ cnat_rec (cnat → cnat) (n ↦ n) (m g n ↦ csuc (g n))
...
def cnat_mul : cnat → (cnat → cnat)
  ≔ cnat_rec (cnat → cnat) (n ↦ czero) (m g n ↦ cnat_add (g n) n)
...
def cnat_mul_succ
  : (i j : cnat) → eq cnat (cnat_mul i (csuc j)) (cnat_add i (cnat_mul i j))
  ≔ cnat_ind
      (i ↦
