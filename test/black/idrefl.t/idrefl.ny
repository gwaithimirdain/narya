...
def refl_id_map_act : Id (Id X x0 x1) (refl ((x ↦ x) : (X → X)) x0 x1 x2) x2
  ≔ refl x2
...
def idff_to_idff' : idff → idff' ≔ h x x' x'' ↦ h x x' x''
...
def ap_is_functorial
  : Id (Id Z (g (f x0)) (g (f x1))) (refl g (f x0) (f x1) (refl f x0 x1 x2))
      (refl ((x ↦ g (f x)) : X → Z) x0 x1 x2)
  ≔ refl (refl ((x ↦ g (f x)) : X → Z) x0 x1 x2)
...
def r1x2' ≔ x2⁽¹ᵉ⁾
def r1x2_eq_r1x2'
  : Id (X⁽ᵉᵉ⁾ x0 x0 (refl x0) x1 x1 (refl x1) x2 x2) r1x2 r1x2'
  ≔ refl r1x2

def r2x2 ≔ (refl ((x ↦ refl x) : (x : X) → Id X x x) x0 x1 x2)
def r2x2' ≔ x2⁽ᵉ¹⁾
def r2x2_eq_r2x2'
  : Id (X⁽ᵉᵉ⁾ x0 x1 x2 x0 x1 x2 (refl x0) (refl x1)) r2x2 r2x2'
  ≔ refl r2x2

{`But applying symmetry identifies both their types and values.`}
def sr1x2_eq_r2x2
  : Id (X⁽ᵉᵉ⁾ x0 x1 x2 x0 x1 x2 (refl x0) (refl x1)) (sym r1x2) r2x2
  ≔ refl r2x2

def sr1x2'_eq_r2x2'
  : Id (X⁽ᵉᵉ⁾ x0 x1 x2 x0 x1 x2 (refl x0) (refl x1)) (x2⁽ᵉ⁾⁽²¹⁾) r2x2'
  ≔ refl r2x2'

{`But the two degenerate squares associated to a reflexivity *are* equal.`}
def r1reflx0 ≔ refl (refl x0)
def r2reflx0 ≔ refl ((x ↦ refl x) : (x : X) → Id X x x) x0 x0 (refl x0)
def r1reflx0_eq_r2reflx0
  : Id (X⁽ᵉᵉ⁾ x0 x0 (refl x0) x0 x0 (refl x0) (refl x0) (refl x0)) r1reflx0
      r2reflx0
  ≔ refl r1reflx0
