...
are instantiated at the boundary's field projections, which are keyed into the
ambient context and so cannot appear behind the right-adjoint lock where the
declaration puts the type.  So "about" falls back to the normal form.

  $ narya -spatial modalfields.ny -e "about (refl C)" -e "about (refl R)"
  codata⁽ᵉ⁾ [
  | (𝑥 :♭| _) .fld : N⁽ᵉ⁾ ((𝑥.0 :♭| _) .fld) ((𝑥.1 :♭| _) .fld) ]
    : Type⁽ᵉ⁾ C C
  
  sig⁽ᵉ⁾ (
    (𝑥 :♭| _) .fst : N⁽ᵉ⁾ ((𝑥.0 :♭| _) .fst) ((𝑥.1 :♭| _) .fst) )
    : Type⁽ᵉ⁾ R R
...
then extended by the self variable, annotated by the left adjoint.
  ...
  $ narya -spatial -v -e "def √♭ (B :♯| Type) : Type ≔ codata [ (x :♭| _) .root.e : ? ]"
   ￫ info[I0000]
...
degeneracy has no declaration form and falls back to the normal form.
  ...
  codata⁽ᵉ⁾ [
  | (𝑥 :♭| _) .root.e : Id A ((𝑥.02 :♭| _) .root) ((𝑥.12 :♭| _) .root)
  | (𝑥 :♭| _) .root.1 : A ]
    : Type⁽ᵉ⁾ √♭A √♭A
...
context degenerated by its own remaining dimensions.
  ...
  codata⁽ᵉᵉ⁾ [
  | (𝑥 :♭| _) .root.e
    : A⁽ᵉᵉ⁾ ((sym 𝑥.022 :♭| _) .root.1) ((sym 𝑥.122 :♭| _) .root.1)
        ((sym 𝑥.202 :♭| _) .root.1) ((sym 𝑥.212 :♭| _) .root.1)
  | (𝑥 :♭| _) .root.1 : Id A ((𝑥.20 :♭| _) .root) ((𝑥.21 :♭| _) .root)
  | (𝑥 :♭| _) .root.2 : Id A ((𝑥.02 :♭| _) .root) ((𝑥.12 :♭| _) .root) ]
