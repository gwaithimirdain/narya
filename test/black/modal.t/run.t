...
adjoint.
  ...
  $ narya -v -e "option modal ≔ spatial def C3 (A :♯| Type) : Type ≔ codata [ (x :♭| _) .fld : ? ]"
   ￫ info[I0000]
...
not sinister.
  ...
  $ narya -e "option modal ≔ spatial def Bad : Type ≔ codata [ | (x :♯| _) .f : Type ]"
   ￫ error[E1711]
   ￭ command-line exec string
   1 | option modal ≔ spatial def Bad : Type ≔ codata [ | (x :♯| _) .f : Type ]
...
If a type is given in a modal field projection, it must be correct.
  ...
  $ narya -e "option modal ≔ coreflection def □′ (A :□| Type) : Disc ≔ sig ((x :△| □′ A) .unbox : A)" -e "def unbox (A :△□| Type) (u :△| □′ A) : A ≔ (u :△| Disc) .unbox"
   ￫ error[E0401]
   ￭ command-line exec string
...
And likewise in a record definition.
  ...
  $ narya -e "option modal ≔ coreflection def □′ (A :□| Type) : Disc ≔ sig ((x :△| Disc) .unbox : A)"
   ￫ error[E1508]
   ￭ command-line exec string
   1 | option modal ≔ coreflection def □′ (A :□| Type) : Disc ≔ sig ((x :△| Disc) .unbox : A)
     ^ invalid self variable type for field unbox: head must be current record
  
  [1]

  $ narya -e "option modal ≔ coreflection def □′ (A :□| Type) : Disc ≔ sig ((x :△| □′ Type) .unbox : A)"
   ￫ error[E1508]
   ￭ command-line exec string
   1 | option modal ≔ coreflection def □′ (A :□| Type) : Disc ≔ sig ((x :△| □′ Type) .unbox : A)
...
adjunction counit.
  ...
  a
   ...
   ￫ info[I0000]
   ￮ constant cfst defined
  
   ￫ info[I0000]
   ￮ constant proot defined
...
The type of a modal higher field is checked in a context that is degenerated by
the field's intrinsic dimension, then locked by the right adjoint, and only
then extended by the self variable, annotated by the left adjoint.

  $ narya -v -e "option modal ≔ spatial def √♭ (B :♯| Type) : Type ≔ codata [ (x :♭| _) .root.e : ? ]"
   ￫ info[I0000]
   ￮ constant √♭ defined, containing 1 hole
...
field's intrinsic dimensions; modal higher fields there are not yet supported.
  ...
  $ narya -e "option parametric ≔ arity 1, letter d option modal ≔ discrete spatial axiom A : Type" -e "def R : Type ≔ codata [ (x :♭| _) .root.d : A ]"
   ￫ error[E0100]
   ￭ command-line exec string
