...
axiom M : (Gel A B R)⁽ᵖ¹⁾ {a₀} {b₀} (r₀,) {a₁} {b₁} (r₁,) a₂ b₂
echo sym M
...
def eta
  : Br ((Gel A B R)⁽ᵖ¹⁾ {a₀} {b₀} (r₀,) {a₁} {b₁} (r₁,) a₂ b₂) M
      (sym (ungel ≔ sym M .ungel))
  ≔ rel M

{` rel of Gel builds a square of correspondences`}
axiom A0 : Type
axiom B0 : Type
axiom R0 : A0 → B0 → Type
axiom A1 : Type
...
def r_gelr2 ≔ rel Gel A2 B2 R2

synth r_gelr2
{`r_gelr2 : Type⁽ᵖᵖ⁾ A0 A1 A2 B0 B1 B2 (Gel A0 B0 R0) (Gel A1 B1 R1)`}
...
def r_sym_gelr2 ≔ sym (rel Gel A2 B2 R2)

synth r_sym_gelr2
{`r_sym_gelr2 : Type⁽ᵖᵖ⁾ A0 B0 (Gel A0 B0 R0) A1 B1 (Gel A1 B1 R1) A2 B2`}
...
def symsym_r2_eq_r2 : Br r2ty symsym_r2 r2 ≔ rel r2

def symsym_r2' ≔ sym (sym r2')

def symsym_r2'_eq_r2' : Br r2ty symsym_r2 r2 ≔ rel r2
