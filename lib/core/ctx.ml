...
let app_entry : type dom modality mode f n any.
 ...
 fun apps e ->
  match e with
  | Vis { bindings; modality; _ } | Invis { modality; bindings; _ } ->
...
module Ordered = struct
  type ('mode, _, _) t =
    | Emp : 'mode Mode.t -> ('mode, N.zero, 'mode emp) t
    | Snoc :
  ...
  let vis : type dom modality mode a b f af m n mn.
   ...
   fun ctx modality filter dim plusdim vars bindings af ->
    Snoc
      ( ctx,
        Vis
          {
  ...
  let cube_vis : type dom modality mode a b n.
   ...
   fun ctx modality filter x vars ->
    let m = CubeOf.dim vars in
    vis ctx modality filter m (D.plus_zero m) (NICubeOf.singleton x) vars (Suc Zero)
  ...
  let invis : type dom modality mode a b n.
   ...
   fun ctx modality filter bindings -> Snoc (ctx, Invis { modality; bindings; filter }, Zero)
  ...
  let rec checked_length : type mode a b. (mode, a, b) t -> (mode, b) Tctx.t = function
    ...
    | Snoc (ctx, e, _) ->
        Tctx.suc (checked_length ctx) (Dim (modality_entry e, dim_entry e, filter_entry e))
  ...
  type (_, _, _, _, _) locked =
    | Locked :
        ('b, 'cod, 'modality, 'dom, 'bm) plus_lock * ('dom, 'a, 'bm) t
        -> ('cod, 'a, 'b, 'modality, 'dom) locked

  let rec lock : type dom modality cod a b.
      (cod, a, b) t -> (dom, modality, cod) Modality.t -> (cod, a, b, modality, dom) locked =
   fun ctx mu ->
    match mu with
    | Path (Zero, mode) -> Locked (plus_no_lock mode, ctx)
    | Path (Suc (modality, mu), mode) ->
        let (Locked (bm, newctx)) = lock ctx (Path (modality, mode)) in
        Locked (plus_lock_suc bm mu, Lock (newctx, mu))

  (* The same, but given a plus_lock that determines the output shape, rather than returning it as an existential variable.  *)

  let rec lock_to : type dom modality cod a b bm.
      (cod, a, b) t ->
      (dom, modality, cod) Modality.t ->
      (b, cod, modality, dom, bm) plus_lock ->
      (dom, a, bm) t =
   fun ctx mu (Plus_lock (l, al)) ->
    match mu with
    | Path (Zero, _) ->
        let (Zero _) = l in
        let Zero = al in
    ...
    | Lookup v -> (
        ...
        | Plus_with_locks (Suc _, _) ->
            Lookup
              {
                v with
                plus = plus_with_locks_dim v.plus modality filter n <|> Anomaly "pop_lookup";
              }
  ...
  and lookup_entry : type dom modality mode a b f af mn.
   ...
   fun ctx e pf k ->
    ...
    | Vis
        (type m n f1 f2)
        ({ dim; modality; filter; plusdim; vars; bindings; hasfields = _; fields; fplus } :
          (dom, modality, mode, m, n, mn, f1, f2, f) vis_data) -> (
    ...
    | Invis { modality; filter; bindings } ->
        let Zero = pf in
  ...
  let rec find_level : type mode a b. (mode, a, b) t -> level -> (mode, b, Empty.t) lookup option =
   fun ctx i ->
    match ctx with
    | Emp _ -> None
    | Snoc (ctx, Vis { bindings; modality; filter; _ }, _) ->
        find_level_in_cube ctx bindings modality filter i
    | Snoc (ctx, Invis { modality; bindings; filter }, _) ->
        find_level_in_cube ctx bindings modality filter i
  ...
  and find_level_in_cube : type dom modality mode a b n.
   ...
   fun ctx vars modality filter i ->
  ...
  let ext : type dom modality mode a b.
   ...
   fun ctx modality x ty ->
    ...
    (cube_vis ctx modality (Modality.filter_zero modality) x (CubeOf.singleton b), b)
  ...
  type ('mode, _, _) pop =
    | Pop :
        ('mode, 'a, 'b) t
        * ('a N.suc, 'asuc) Eq.t
        * (('b, ('modality, 'n) dim_entry) snoc, 'bn) Eq.t
        -> ('mode, 'asuc, 'bn) pop
  ...
  type (_, _, _, _) remove_lock =
    | Remove_lock :
        ('cod, 'a, 'b) t * ('b, 'cod, 'modality, 'mode, 'bc) plus_with_locks
        -> ('cod, 'modality, 'mode, 'bc) remove_lock

  let rec remove_lock : type mode modality cod a bc.
      (mode, a, bc) t -> (mode, modality, cod) Modality.t -> (cod, modality, mode, bc) remove_lock =
   fun ctx modality ->
    match Modality.compare_id modality with
    | Eq -> Remove_lock (ctx, plus_with_no_locks (mode ctx))
    | Neq -> (
        match ctx with
        ...
        | Snoc (ctx, e, _) -> (
            ...
            match plus_with_locks_dim plus (modality_entry e) (dim_entry e) (filter_entry e) with
...
let vis (Permute { perm; env; level; ctx }) modality filter m mn xs vars af =
  ...
  Permute
    {
      ...
      level = level + 1;
      ctx = Ordered.vis ctx modality filter m mn xs vars bf;
    }
...
let variables_vis : type dom modality mode a b mn.
 ...
 fun ctx modality filter (Variables (m, mn, xs)) vars ->
  let (Plus af) = N.plus (NICubeOf.out N.zero xs) in
  Any_ctx (vis ctx modality filter m mn xs vars af)

let cube_vis ctx modality filter x vars =
  let m = CubeOf.dim vars in
  vis ctx modality filter m (D.plus_zero m) (NICubeOf.singleton x) vars (Suc Zero)
...
let invis (Permute { perm; env; level; ctx }) modality filter vars =
  Permute
    {
      perm;
      env =
      ...
      level = level + 1;
      ctx = Ordered.invis ctx modality filter vars;
    }
