  ...
  and act_apps : type hmode mode mu1 mu2 cod a b any.
   ...
   fun apps s c ->
    ...
    (* A specialization neither crosses a mode nor carries an insertion, so the degeneracy passes straight through to the rest of the spine, and the constructor it stores is acted by that same degeneracy.
       ...
       It is unreachable, for the following reason.  Across the whole test suite every action on a specialization is the identity degeneracy carrying a modality cell -- the modal-field readbacks in about.t -- where act_value and act_ty agree.  The one site that acts a self by anything else is readback_at's permuted-record case, which un-permutes a struct at a codatatype whose insertion is not the identity, and it acts the self only at *potential* energy.  But a potential struct is a comatch node of a case tree, and check refuses to comatch, or to check a tuple, against a codatatype carrying a nonidentity degeneracy (Comatching_at_degenerated_codata and Checking_tuple_at_degenerated_record, in check_struct).  So a permuted struct is only ever a kinetic value, obtained by acting an unpermuted one -- "sym (ungel ≔ …)" -- and is read back at kinetic energy, where the self is not touched.  If that ever changes, the fix is to stop storing the type rather than to patch here, since act_apps will still have no term to hand act_ty. *)
