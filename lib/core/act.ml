...
module Act = struct
  ...
  and act_apps : type hmode mode mu1 mu2 cod a b any.
   ...
   fun apps s c ->
    ...
    (* A specialization neither crosses a mode nor carries an insertion, so the degeneracy passes straight through to the rest of the spine, and the constructor it stores is acted by that same degeneracy, under a cell prewhiskered by the window modality it lives behind -- as an argument's is by its own.

       It carries no type to act on, tyof_specialize computing that from the match instead, which is what keeps the type of an acted specialization right.  Acting a stored one is what act_apps cannot do: the type of an acted term is act_ty's business, and act_ty needs the term, which a spine entry is acted without. *)
