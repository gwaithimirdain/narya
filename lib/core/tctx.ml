...
module VarAnnote = struct
  ...
  (* The "pattern_name" is the display name (or names) of the variable, carried so that it can be recovered when reading back/unparsing a match branch.  It is irrelevant to the type indices and to all categorical operations (src/tgt/compare). *)
  type (_, _, _) t =
    | Annote : pattern_name * ('dom, 'modality, 'mode) Modality.t -> ('mode, 'modality, 'mode) t
...
module VarAnnotator = struct
  ...
  type (_, _, _, _, _, _, _) t =
    | Annotate :
        pattern_name * ('dom, 'modality, 'mode, 'k, 'n) Modality.filter_dim
...
let rec annotate_names : type n mode annotations b.
    (n, mode, annotations, mode, mode, b, mode) VarAnnotate.fwd_t -> pattern_name list = function
  | Zero _ -> []
