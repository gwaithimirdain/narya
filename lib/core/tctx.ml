...
module TEntry = struct
  ...
  type (_, _, _) t =
    | Dim :
        ...
        * 'a D.t
        * ('dom, 'modality, 'mode, 'a, 'a) Modality.filter_dim
  ...
  let src_uniq : type src1 m tgt1 src2 tgt2.
   ...
   fun m1 m2 ->
    match (m1, m2) with
    | Dim (m1, _, _), Dim (m2, _, _) -> Modality.tgt_uniq m1 m2
...
module Locksmap = struct
  ...
  type (_, _, _, _, _, _) t =
    | Locks_dim :
        ...
        * 'a D.t
        * ('dom, 'annote, 'mode, 'a, 'a) Modality.filter_dim
  ...
  let exists : type a m b. (a, m, b) Dom.t -> (a, m, b) exists = function
    | Dim (x, a, f) -> Exists (Locks_dim (x, a, f))
...
let plus_with_locks_dim : type a cod modality mode dom ac annote n.
 ...
 fun (Plus_with_locks (c, l)) annote n f ->
  Some (Plus_with_locks (Suc (c, Dim (annote, n, f)), Suc (l, Locks_dim (annote, n, f), Zero)))
...
module VarAnnotator = struct
  ...
  type (_, _, _, _, _, _, _) t =
    | Annotate :
        ('dom, 'modality, 'mode) Modality.t * ('dom, 'modality, 'mode, 'k, 'n) Modality.filter_dim
  ...
  let uniq : type param a m b x1 n1 y1 x2 n2 y2.
   ...
   fun (Annotate (m1, f1)) (Annotate (m2, f2)) ->
    let Eq = Modality.src_uniq m1 m2 in
...
let rec insert_dom : type a x n b mode. (a, x, n, b) insert -> (mode, b) Tctx.t -> (mode, a) Tctx.t
 ...
 fun i b ->
  ...
  | Now ->
      ...
      Path (a, mode)
  | Later i ->
      let (Path (Suc (b, Dim (x, n, f)), mode)) = b in
      let a = insert_dom i (Path (b, mode)) in
      Tctx.suc a (Dim (x, n, f))
...
let rec inserted : type a modality n b mode.
 ...
 fun i b ->
  match i with
  | Now ->
      let (Path (Suc (_, Dim (modality, n, f)), _)) = b in
      Inserted (modality, n, f)
  | Later i ->
...
let rec insert_from_comp : type b m n a ab ac w x y z.
 ...
 fun m n f i ab ->
  ...
  match i with
  | Now -> return (Insert_from_comp (Suc (ab, Dim (m, n, f)), Now))
  | Later i -> (
      match ab with
      | Suc (ab, Dim (p, k, f2)) ->
          let* (Insert_from_comp (ac, j)) = insert_from_comp m n f i ab in
          return (Insert_from_comp (Suc (ac, Dim (p, k, f2)), Later j))
      | Zero -> None)
...
let rec insert_locks : type a p n b w x y m.
 ...
 fun p n f i l ->
  match (i, l) with
  | Now, _ -> Suc (l, Locks_dim (p, n, f), Zero)
  | Later i, Suc (l, Locks_dim (p', n', f'), Zero) ->
      Suc (insert_locks p n f i l, Locks_dim (p', n', f'), Zero)
...
let rec perm_dom : type mode a b. (a, b) permute -> (mode, b) Tctx.t -> (mode, a) Tctx.t =
 fun p b ->
  ...
  | Insert (p, i) ->
      ...
      let (Inserted (modality, n, f)) = inserted i b in
      Tctx.suc a (Dim (modality, n, f))
  | Lock p ->
...
type (_, _, _, _, _) fwd_insert =
  | Now :
      ('u, 'x, 'mode) Modality.t * 'n D.t * ('u, 'x, 'mode, 'n, 'n) Modality.filter_dim
...
let rec fwd_insert_cod : type u v a x n b.
 ...
 fun i a ->
  match i with
  | Now (x, n, f) -> Cons (Dim (x, n, f), a)
  | Later (y, i) ->
      let (Cons (Dim (k, n, f), a)) = a in
      let Eq = Modality.tgt_uniq y k in
      Cons (Dim (k, n, f), fwd_insert_cod i a)
...
let rec bcomp_fwd_inserted : type x y z a b c cb d m n.
 ...
 fun cb bins fins ->
  ...
  | Now (m, n, f) ->
      ...
      Bcomp_fwd_inserted (Suc (Dim (m, n, f), ad), perm_bcomp ad cb (Insert (Id, bins)))
  | Later (m1, fins) ->
      let (Suc (Dim (m2, n, f), cb)) = cb in
      let Eq = Modality.tgt_uniq m1 m2 in
      let (Bcomp_fwd_inserted (ad, p)) = bcomp_fwd_inserted cb (Later bins) fins in
      Bcomp_fwd_inserted (Suc (Dim (m2, n, f), ad), p)
...
let rec unpermute_plus_locks : type a c ac bd mode mu dom p.
 ...
 fun p ac lc ->
  ...
  | Insert (p, i), Suc (ac, Dim (m, k, f)), Suc (lc, Locks_dim _, Zero) ->
      let* (Unpermute (p, bd, ld)) = unpermute_plus_locks p ac lc in
      let* (Insert_from_comp (bd, j)) = insert_from_comp m k f i bd in
      let ld = insert_locks m k f j ld in
...
module Anyplus = struct
  ...
  let dom : type param a m b x n y. (param, a, m, b, x, n, y) t -> (a, m, b) Dom.t = function
    | Plus_dim (x, a, fn, _) -> Dim (x, D.plus_right a, fn)
    | Plus_lock m -> Lock m
    | Plus_proj x -> Proj x
  ...
  let cod : type param a m b x n y. param Param.t -> (param, a, m, b, x, n, y) t -> (x, n, y) Cod.t
   ...
   fun p -> function
    | Plus_dim (x, a, fn, fq) ->
        ...
        Dim (x, D.plus_out q a, Modality.filter_plus a a (Modality.filter_idempotent fq) fn)
    | Plus_lock m -> Lock m
    | Plus_proj x -> Proj x

  let src : type param a m b x n y. (param, a, m, b, x, n, y) t -> (param, a, x) Obj.t = function
    | Plus_dim (x, _, _, _) -> Eq (Mode (Modality.tgt x))
  ...
  let exists : type param a m b. param Param.t -> (a, m, b) Dom.t -> (param, a, m, b) exists =
   fun p -> function
    | Dim (x, a, fn) ->
        let (Has_filter fq) = Modality.filter x p in
        let (Plus qa) = D.plus a in
        Exists (Plus_dim (x, qa, fn, fq))
  ...
  let uniq : type param a m b x1 n1 y1 x2 n2 y2.
   ...
   fun f1 f2 ->
    match (f1, f2) with
    | Plus_dim (m1, n1, fn1, fq1), Plus_dim (m2, n2, fn2, fq2) ->
        let Eq = Modality.src_uniq m1 m2 in
...
module Plusmap = struct
  ...
  let rec zerol : type x bs y. (x, bs, y) Dom.t -> (D.zero, x, bs, y, x, bs, y) t = function
    ...
    | Path (Suc (bs, Dim (x, b, f)), mode) ->
        Suc
          ( zerol (Path (bs, mode)),
            Inject (Plus_dim (x, D.zero_plus b, f, Modality.filter_zero x)),
            Suc (Zero, Dim (x, b, f)) )
    ...
    | Path (Suc (Suc (_, Dim (modality, _, _)), Proj _), _) ->
        Mode.not_unit (Modality.tgt modality) Eq
  ...
  let rec locks : type n x y b nb m.
   ...
   fun n n_b ll_b ->
    ...
    | Suc (n_b, Inject (Plus_dim (mu, qm, fm, fq)), Suc (Zero, _)), Suc (ll_b, Locks_dim _, Zero) ->
        let q = Modality.filtered n fq in
        let fqm = Modality.filter_plus qm qm (Modality.filter_idempotent fq) fm in
        Suc (locks n n_b ll_b, Locks_dim (mu, D.plus_out q qm, fqm), Zero)
  ...
  let rec unlocks : type n x y b nb m.
   ...
   fun n_b ll_nb ->
    ...
    | Zero _, Zero m -> Zero m
    | Suc (n_b, Inject (Plus_dim (mu, qm, fm, _)), Suc (Zero, _)), Suc (ll_nb, Locks_dim _, Zero) ->
        Suc (unlocks n_b ll_nb, Locks_dim (mu, D.plus_right qm, fm), Zero)
  ...
  let rec uninsert : type p a b m xs x ys ws.
   ...
   fun i pxs ->
    match (pxs, i) with
    | Suc (pxs, Inject (Plus_dim (_, qx, _, fq)), Suc (Zero, _)), Now -> Uninsert (qx, fq, Now, pxs)
    | Suc (pxs, Inject (Plus_dim (m, px, filtern, filterq)), Suc (Zero, yy)), Later i ->
        let (Uninsert (u, f, fi, pxs)) = uninsert i pxs in
        Uninsert
          (u, f, Later fi, Suc (pxs, Inject (Plus_dim (m, px, filtern, filterq)), Suc (Zero, yy)))
  ...
  let rec uncoinsert : type p a b m ys z zs ws.
   ...
   fun i pxs ->
    match i with
    | Now ->
        let (Suc (pxs, Inject (Plus_dim (_, qx, _, fq)), Suc (Zero, _))) = pxs in
        Uncoinsert (qx, fq, Now, pxs)
    | Later i ->
        let (Suc (pxs, Inject (Plus_dim (m, px, filtern, filterq)), Suc (Zero, yy))) = pxs in
        let (Uncoinsert (px', f, fi, pxs)) = uncoinsert i pxs in
        Uncoinsert
          (px', f, Later fi, Suc (pxs, Inject (Plus_dim (m, px, filtern, filterq)), Suc (Zero, yy)))
end
