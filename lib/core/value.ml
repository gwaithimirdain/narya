...
module rec Value : sig
  ...
  and (_, _, _) apps =
    ...
    (* A display-only spine entry, never produced by typechecking: it specializes a stuck match at the head end of the spine, reducing it as if its discriminee were the stored constructor value.  Readback attaches one to the self it hands a branch body, so that the self's spine really does evaluate to that body -- which projecting a higher codata field from it, and hence degenerating it, requires.  It stores the constructor as a normal -- a constructor does not synthesize, so reading one back needs its type -- and the type of the specialized match, which is the type the branch was checked at.
       ...
       Carrying that type rather than computing it from the match is load-bearing, not an expedient.  Computing it means applying the motive to the branch's indices and constructor after the eval-readback cycle, and that cycle happens in the context degenerated by a higher field's remaining dimensions; there the motive has to be instantiated at the boundary of the branch body, which for a comatch body is a case tree rather than a term, so the computation gives up where the stored type does not.  The stored type is computed once in the undegenerated environment, where no boundary is needed, and eval degenerates it.  Computing becomes possible once a branch body's boundary can be supplied at a face -- which a specialization at that face would be, if faces of a neutral were reachable.  (Measured: neither version costs anything, on typechecking or on display.)
  ...
  and (_, _, _) canonical =
    ...
    | Codata : ('mode, 'm, 'n, 'a, 'et) codata_args -> ('mode, 'm, 'n) canonical
  ...
  and ('mode, 'm, 'n, 'a, 'et) codata_args = {
    eta : (potential, 'et) eta;
    opacity : opacity;
    hints : hints;
    env : ('mode, 'm, 'a) env;
    fields : ('mode * 'a * D.zero * 'n * 'et) Term.CodatafieldAbwd.t;
  }
