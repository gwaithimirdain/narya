...
type (_, _, _) checkable_branch =
  | Checkable_branch : {
      ...
      env : ('mode, 'm, 'b) env;
      (* The value of the datatype's self-type variable, carried along with the environment because the constructor's type is stored over it. *)
      self : ('m, ('mode, kinetic) lazy_eval) CubeOf.t;
      ty : ('mode, ('b, ('mode Modality.id, D.zero) dim_entry) snoc, kinetic) term;
    }
 ...
 fun head user_branches data_constrs ->
  let user_branches, leftovers =
    Bwd.fold_left
      (fun ((userbrs, databrs) :
        ...
        let (Value.Dataconstr { env; self; ty }) =
          match databr with
          | Some db -> db
        ...
        | Eq ->
            let br = Checkable_branch { xs; body = Some body; env; self; ty } in
  ...
  Bwd.prepend user_branches
    (Bwd_extra.to_list_map
       (fun (c, Value.Dataconstr { env; self; ty }) ->
         let (Wrap arity) = pi_arity ty in
         let (Bplus plus_args) = Raw.Indexed.bplus arity in
         let xs = Namevec.none plus_args in
         (c, Checkable_branch { xs; body = None; env; self; ty }))
       leftovers)

(* Extend a context by the self-type variable of a datatype: an invisible variable standing for the datatype's own type family, of the type that family is being checked against (e.g. "N → Type" for "Vec A").  Every constructor's arguments and output type are checked over it, and its codomain names the datatype by it rather than by the head being checked, so that evaluation and readback can each say what the datatype is.  Being invisible, it gives the user no way to refer to the datatype, so it is not a route to recursion. *)
let self_var : type mode a b.
    (mode, a, b) Ctx.t ->
    (mode, kinetic) value ->
    (mode, a, (b, (mode Modality.id, D.zero) dim_entry) Tbwd.snoc) Ctx.t * (mode, kinetic) value =
 fun ctx ty ->
  let mode = Ctx.mode ctx in
  let vars, nfs = dom_vars ctx (Modality.id mode) (CubeOf.singleton ty) in
  (Ctx.invis ctx (Modality.filter_id mode D.zero) nfs, CubeOf.find_top vars)

(* Build a constructor's codomain over the datatype's self-type variable: that variable applied to this constructor's indices, as the datatype would be applied to them.  This is what the codomain names the datatype by, whether or not the user declared an output type; when one is declared, the indices are the ones get_indices extracts from it. *)
let self_codomain : type mode a b.
    (mode, a, b) Ctx.t ->
    (mode, kinetic) value ->
    (mode, b, kinetic) term list ->
    (mode, b, kinetic) term =
 fun ctx selfval indices ->
  let mode = Ctx.mode ctx in
  List.fold_left
    (fun t i ->
      Term.App
...
(* The self variable of a codatatype or record type belongs to the codatatype-so-far, but it is annotated by a modality that varies from field to field: the left adjoint of the field's adjunction (the identity for an ordinary field).  Since that annotation is recorded in the variable itself, with_codata_so_far hands its continuation this polymorphic function to create the self variable at whatever annotation the field in question requires. *)
type (_, _) self_vars =
  | Self_vars : {
      self :
        'f 'g 'gmode.
 ...
 fun ?discrete status ctx loc { value = constr; loc = constr_loc } args ty ->
  ...
  | Canonical
    ...
    -> (
      ...
      | Some (Dataconstr { env; self; ty = constr_ty }) ->
          ...
          let out, newargs =
            check_at_pi constr ctx (dim_env env)
              (eval_term (dataconstr_env env self) constr_ty)
              args tyarg_args in
  ...
  | { value = Var ix; loc } -> (
      ...
      | Zero, Zero _ -> (
          ...
          | `Var (Some level, fa) ->
              ...
              check_var_match status ctx level index (Lazy.force varty) modality plus brs refutables
                highers motive loc))
  | _ -> fallback ()
    ...
    (* Data always has intrinsic dimension zero, but it seems that the syntax for binding type variables in a GADT match can't take that into account.  So we have to give "zero" a name here, and two different names for m. *)
      ...
      let branches, errs =
        List.fold_left
          (fun (branches, errs)
               ( constr,
                 (Checkable_branch { xs; body; env; self; ty } : (dom, a, m) checkable_branch) ) ->
            ...
            let (Ext_pi { ctx = newctx; values = newvars; normals = newnfs; annotate; comp; out }) =
              ext_pi ctx window env xs (eval_term (dataconstr_env env self) ty) in
      ...
      let get : type m.
       ...
       fun dim user_branches _ ->
        ...
        let synth_branches, check_branches =
          List.partition_map
            (fun (c, (Checkable_branch { xs; body; env; self; ty } as cb)) ->
              match body with
              | Some { value = Synth sbody; loc } ->
                  let body = locate_opt loc sbody in
                  Left (c, Synthable_branch { xs; body; env; self; ty })
              | _ -> Right (c, cb))
            user_branches in
        ...
        let rec find_synthing_branch errs = function
          ...
          | (constr, (Synthable_branch { xs; body; env; self; ty } : (dom, a, m) synthable_branch))
            :: brs ->
        ...
        let check_branches =
          List.fold_right
            (fun (c, Synthable_branch { xs; body; env; self; ty }) cbs ->
              let body = Some { value = Synth body.value; loc = body.loc } in
              (c, Checkable_branch { xs; body; env; self; ty }) :: cbs)
          ...
          (Motive
             {
               ...
               use =
                 (fun (_, emotive) constr dim index_vals newvars ->
                   ...
                   let constr_vals = constr_val_cube constr dim newvars in
      ...
      let branches, errs =
        List.fold_left
          (fun (branches, errs)
            ...
            let index_nfs = indices_of_out "match branch" out dim (Vec.length var_indices) in
            ...
            let constr_nfs =
              constr_norm_cube (Modality.src window) constr dim tyfam index_vals newvars in
            ...
            Vec.miter
              (fun [ vs; cs ] ->
                CubeOf.miter { it = (fun _ [ v; c ] -> Hashtbl.add new_vals v c) } [ vs; cs ])
            ...
            | Bind_some { checked_perm; oldctx; newctx } -> (
                ...
                let status =
                  make_match_status status window plus (Term.Var index) dim None branches annotate
                    comp
                    (Some (oldctx, newctx))
...
and check_data : type mode a b i.
    ...
    (mode, kinetic) value ->
    i Fwn.t ->
    (Constr.t, (mode, (b, (mode Modality.id, D.zero) dim_entry) snoc, kinetic) term) Abwd.t ->
  ...
  | ( (c, { value = Dataconstr (args, output); loc }) :: raw_constrs,
      ...
      | None, Some output ->
          let ( disc,
            ...
            @@ fun () ->
            (* The constructor's arguments and output are checked over the datatype's self-type variable, which its stored codomain names the datatype by.  The output type is NOT included in the positivity scope: its head is by definition the current datatype, and occurrences there are not recursion. *)
            let selfctx, selfval = self_var ctx ty in
            let (Checked_tel (args, newctx), disc), crec =
              Positivity.scope @@ fun () -> check_tel ?discrete selfctx args in
                    ...
                    | Zero -> (
                        ...
                        | Eq ->
                            ( disc,
                              ...
                              |> Abwd.add c
                                   (Telescope.pis args
                                      (self_codomain newctx selfval (Vec.to_list indices))),
                              errs )
                        | _ ->
      ...
      | None, None -> (
          ...
          | Zero ->
              let ( disc,
                ...
                let (Checked_tel (args, newctx), disc), crec =
                  Positivity.scope @@ fun () -> check_tel ?discrete selfctx args in
                (* In this case, the output of the constructor is the datatype itself, which is what the self-type variable stands for. *)
                let output = self_codomain newctx selfval [] in
 ...
 fun (Potential (h, args, _, hyp)) eta ctx opacity hints dim tyargs checked_fields
  ...
  let domvars =
    ...
    | Emp ->
        ...
        (* The self variable, annotated by the left adjoint f of a specified adjunction, in the context locked by the right adjoint g (which is where it will be added).  Only the level of that context matters to dom_vars, and locking doesn't change that.  But the *type* of such a variable lives in the context locked by g and then by f, whereas the codatatype-so-far lives in the unlocked context; we transport it there along the adjunction unit 1 ⇒ gf, which is exactly the substitution that MTT associates to that 2-cell.  For an ordinary field the unit is an identity cell and this is a no-op. *)
        let domvars : type f g gmode.
            (mode, f, g, gmode) Modalcell.adjunction -> (n, mode Ctx.Binding.t) CubeOf.t =
         fun adj ->
          let (Locked (_, lctx)) = Ctx.lock ctx (Modalcell.adj_right adj) in
          let (Adjunction { unit; _ }) = adj in
          let tys =
            CubeOf.mmap { map = (fun _ [ ty ] -> act_value ty (id_deg D.zero) unit) } [ selftys () ]
          in
  ...
  | (Wrap fld, Codatafield (x, lock, self_ty, rty)) :: raw_fields -> (
      ...
      | Zero, _, _ ->
          ...
          let checked_fields, fibrancy, errs =
            ...
            (* The type of a modal field is checked in a context locked by the right adjoint and then extended by the self variable, which is annotated by the left adjoint. *)
            let (Locked (plus_lock, lctx)) = Ctx.lock ctx right in
            (* Since the self variable is annotated by the left adjoint, its dimensions are filtered by it; but the self variable has the dimension of the codatatype, so for now we require that dimension to be unfiltered.  (A nonparametric modal field of a zero-dimensional codatatype is fine: it is at higher *substitution* dimensions that such a field disappears, which is handled elsewhere.) *)
            let (Has_filter left_filter) = Modality.filter left dim in
            match Modality.filter_is_trivial dim left_filter with
            | None ->
                fatal (Unimplemented "nonparametric modal fields of higher-dimensional codatatypes")
            | Some Eq ->
                let newctx =
      ...
      | Pos _, Zero, Noeta ->
          ...
          let checked_fields, errs =
            ...
            match
              (Modality.filter_is_trivial i left_filter, Modality.filter_is_trivial i right_filter)
            with
  ...
  | Ext (Some name, modality, rty, raw_fields) ->
      ...
      let checked_fields, fibrancy, ctx_fields, errs =
        ...
        | Ok (Wrap modality) -> (
            ...
            | Eq ->
                let selfvars = self (Modalcell.id_adjunction (Ctx.mode ctx)) in
                let newctx = Ctx.vis_fields ctx vars selfvars ctx_fields fplus af in
                     ...
                     CubeOf.pmap
                       {
                         map =
                           (fun _ [ x ] ->
                             [ readback_nf lctx x; readback_val lctx (Lazy.force x.ty) ]);
                       }
        ...
        | Eq, _ ->
            ...
            ( realize status
                (Term.Key { tm; cell = Modalcell.id modality; plus_tgt; plus_src }
                  : (mode, b, kinetic) term),
              (act_value (Lazy.force ty) (id_deg D.zero) (Modalcell.id2 mode)
                : (mode, kinetic) value) )
        | _, Some (Unique cell) ->
