        ...
        | Canonical
            ...
            | `Normal -> (
                ...
                | Wrap (names, Ok (af, body)) ->
                    ...
                    let ctx = Ctx.vis ctx filter D.zero (D.zero_plus m) names newnfs af in
                    Lam (xs, n, filter, check ?discrete (mkstatus xs status) ctx body output)
