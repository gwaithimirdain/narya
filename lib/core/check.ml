    ...
    | Constr ({ value = constr; loc = constr_loc }, args), _ -> (
        ...
        | Canonical
            ...
            | Some (Dataconstr { env; ty = constr_ty }) ->
                ...
                | [] -> ()
                | _ :: _ -> (
                    match view_term (Lazy.force out) with
                    | Neu { value; _ } -> (
                        ...
                        | Val
                            (Value.Canonical
                               {
                                 canonical =
                                   Data { dim = outdim; indices = Filled constr_indices; _ };
                                 _;
                               }) -> (
                            match
                              ( D.compare outdim (dim_env env),
                                Fwn.compare (Vec.length constr_indices) (Vec.length ty_indices) )
                            with
                            | Neq, _ ->
                                fatal
                                  (Dimension_mismatch
                                     ("checking constr indices", outdim, dim_env env))
                            | _, Neq ->
                                fatal (Anomaly "length of indices mismatch in checking constr")
                            | Eq, Eq ->
                                Vec.miter
                                  (fun [ t1s; t2s ] ->
                                    CubeOf.miter
                                          ...
                                          (fun fa [ t1; t2 ] ->
                                            ...
                                            | Error err -> (
                                                ...
                                                | None ->
                                                    fatal
                                                      (Anomaly
                                                         "mismatching lower-dimensional constructors")
                                                ));
                                      }
                                      [ t1s; t2s ])
                                  [ constr_indices; ty_indices ])
                        | _ -> fatal (Anomaly "constr output is not a datatype"))
                    | _ -> fatal (Anomaly "constr output is not neutral")));
      ...
      let branches, errs =
        List.fold_left
          (fun (branches, errs)
            ...
            let index_vals = indices_of_out out dim (Vec.length indices) in
            let perm = id_perm in
            let status =
...
and synth_nondep_match : type mode a b.
 ...
 fun status ctx tm window_name brs highers i ->
  ...
  | Wrap (type dom window) (window : (dom, window, mode) Modality.t) -> (
      ...
      let result, motive =
        check_match_branches status ctx tm varty window plus_lock brs i highers loc
          (Motive { get; use = (fun x _ _ _ _ -> x); return = (fun _ _ x -> x) }) in
      match motive with
          ...
          (Motive
             {
               ...
               use =
                 (fun emotive constr dim index_vals newvars ->
          ...
          (fun (branches, errs)
            ...
            let index_vals =
              Vec.map
                ...
                (indices_of_out out dim (Vec.length var_indices)) in
...
and check_at_pi : type mode n a c e.
    ...
    (mode, kinetic) value Lazy.t * (n, mode, e, kinetic) any_modal_term_cube list =
 fun c ctx n fnty tms tyargs ->
  ...
  | tm :: tms, tyargs :: tyargs_rest ->
      ...
      let (Viewed_pi { x = _; filter; doms; cods }) =
        view_constr_pi "check_at_pi" n (Lazy.force fnty) in
      let modality = Modality.filter_modality filter in
      let (Locked (eplus, lctx)) = Ctx.lock ctx modality in
      let tyarg = constr_boundary_tube "check_at_pi" n filter doms tyargs in
      ...
      let out, newargs =
        check_at_pi c ctx n (lazy (apply_binder_term b filter argcube)) tms tyargs_rest in
