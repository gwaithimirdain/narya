...
module rec Make : functor (I : Indices) -> sig
  ...
  and _ check =
    ...
    | Data : (Constr.t, 'a dataconstr located) Abwd.t * string list -> 'a check
    | Codata : (Field.wrapped, 'a codatafield) Abwd.t * string list -> 'a check
    | Record :
        ('a, 'c, 'ac) Namevec.t located * ('ac, 'd, 'acd) tel * opacity * string list
        -> 'a check
    | SelfRecord : (Field.wrapped, 'a codatafield) Abwd.t * string list -> 'a check
