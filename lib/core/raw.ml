...
module rec Make : functor (I : Indices) -> sig
  ...
  type _ synth =
    ...
    | Match : {
        ...
        sort :
          [ `Implicit
          | `Nested of 'a nested_later
          | `Explicit of 'a check located
          | `Nondep of int located ];
  ...
  (* The pattern variables that come after a nested match's discriminee in the constructor pattern it came from.  None of them has been matched on yet when that match is checked, so their types are as the constructor introduced them, and it is exactly those types that the convoy's motive quantifies over -- and those variables that it is applied back to. *)
  and 'a nested_later = 'a synth located list
    ...
    let newtm : a2 R.T2.synth =
      ...
      | Match { tm; window; sort; branches; refutables = r; highers } ->
          ...
          let sort =
            ...
            | `Nondep i -> `Nondep i
            | `Implicit -> `Implicit
            | `Nested later -> `Nested (List.map (synth ctx) later) in
