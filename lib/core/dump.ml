...
module F = struct
  ...
  and evaluation : type mode s. int -> formatter -> (mode, s) evaluation -> unit =
   fun depth ppf v ->
    ...
    | Unrealized (Some (h, i, a)) -> fprintf ppf "Unrealized (%a, %a, %a)" head h apps i apps a
  ...
  and term : type mode b s. formatter -> (mode, b, s) term -> unit =
   fun ppf tm ->
    ...
    | Unapply tm -> fprintf ppf "Unapply (%a)" term tm

  and canonical : type mode b. formatter -> (mode, b) canonical -> unit =
   fun ppf c ->
    match c with
    | Data { indices; evaldim = _; constrs; discrete = _; recursive = _; hints = _; tyfam = _ } ->
