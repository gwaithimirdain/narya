...
let rec get_pi_vars : type a b.
    (a, b) Ctx.t -> [ `Cube | `Normal ] -> binder_name Bwd.t -> kinetic value -> binder_name Bwd.t =
 fun ctx cube xs ty ->
   ...
   fun env tel acc ->
    ...
    | Ext (_, rty, rest) ->
        ...
        let doms =
          CubeOf.build m { build = (fun fa -> Norm.eval_term (act_env env (op_of_sface fa)) rty) }
        in
        let newvars, _ = dom_vars ctx doms in
        let hints = View.hints_of_ty (CubeOf.find_top doms) in
        go (Ext (env, D.plus_zero m, Ok newvars)) rest (Snoc (acc, hints)) in
  go env args Emp
