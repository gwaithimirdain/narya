...
and readback_at : type mode a z.
    ?eta:bool -> (mode, z, a) Ctx.t -> (mode, kinetic) value -> (mode, kinetic) value -> (mode, a, kinetic) term =
...
and readback_val : type mode a z.
    ?sort:[ `Type | `Function | `Other ] -> (mode, z, a) Ctx.t -> (mode, kinetic) value -> (mode, a, kinetic) term =
          ...
          CodCube.build k
            {
              build =
                (fun fa ->
                  ...
                  Cod (readback_val ~sort:`Type sctx (apply_binder_term (BindCube.find cods fa) sargs)));
            } )
...
let readback_bindings : type mode a b n.
    (mode, a, (b, n) snoc) Ctx.t -> (n, mode Binding.t) CubeOf.t -> (n, (mode, (b, n) snoc) binding) CubeOf.t =
 fun ctx vbs ->
  CubeOf.mmap
    {
      map =
...
let readback_entry : type mode a b f n.
    (mode, a, (b, n) snoc) Ctx.t -> (mode, f, n) Ctx.entry -> (mode, b, f, n) entry =
 fun ctx e ->
  match e with
