...
(* A context together with its degeneration by 'r: what deg_nf reads back and re-evaluates in. *)
type (_, _) deg_ctx =
  | Deg_ctx : ('mode, 'raw, 'fb) Ctx.t * ('mode, 'r, 'fb) env -> ('mode, 'r) deg_ctx

(* Where the values of the environment being degenerated live.  Ordinarily they are ambient, and we are given the degeneration of the ambient context: this is the case when checking a higher field, and when reading back a comatch.  But when "about" displays a codatatype, the environment's self entry is the displayed context's own self variable; we are then given that variable's cube, from which deg_env rebuilds the context it lives in, along with that context locked by the field's left adjoint and its degeneration, where the field type's instantiation arguments -- projections of that same self variable -- are read back. *)
type (_, _, _) deg_source =
  | Ambient : ('mode, 'r, 'b) env -> ('mode, 'r, 'b) deg_source
  | Display :
      ('n, 'mode Ctx.Binding.t) CubeOf.t * ('mode, 'raw, 'fb) Ctx.t * ('mode, 'r, 'fb) env
      -> ('mode, 'r, 'b) deg_source
...
and readback_stuck_match : type mode a z.
    (mode, potential) readback_status ->
    (mode, z, a) Ctx.t ->
    (mode, kinetic) value ->
    mode potential_neu ->
    (mode, a, potential) term option =
 fun status ctx ty (Potential_neu (Stuck (env, ctm), apps)) ->
  match ctm with
  | Match { tm; window; plus_lock; dim = match_dim; motive; branches } -> (
      ...
      (* eval records a stuck match in exactly two situations: the discriminee viewed as a constructor whose branch was refuted, and the discriminee not viewing as a constructor at all.  So a constructor here means a refuted branch: the branch has no body, but it does record the term of empty type that refuted it, and since the discriminee really is the constructor we can build the branch's environment from its actual arguments, exactly as evaluation would.  Evaluating that term there and reading it back gives the refutation itself, which is what this whole match amounts to in the inconsistent context we must be in.  Anything other than a constructor or a neutral would be a discriminee that is not an element of a datatype, which the match could not have been checked with. *)
      match (view_term disc, disc) with
      | Constr (cname, constr_dim, dargs), _ -> (
          match (Constr.Map.find_opt cname branches, D.compare constr_dim total_dim) with
          | Some (Term.Refute { annotate; comp; perm; tm = r }), Eq ->
              let bodyenv = take_args env plus_dim dargs window fw annotate comp in
              (* The refutation carries exactly the data a match takes for its discriminee, window and all, so displaying it is just assembling a match with no branches. *)
              let (Refutation { window = rwindow; plus_lock = rlock; tm = rtm }) =
                readback_refutation ctx (Permute (perm, bodyenv)) r in
              Some
                (rewrap
                   (Term.Match
                      {
          ...
          | _ -> no_display "a stuck match on a constructor whose branch is not refuted")
      | _, (Lam _ | Struct _ | Constr _) ->
          fatal (Anomaly "discriminee of a stuck match is not a neutral")
      | _, Neu { ty = (lazy discty); _ } -> (
            ...
            -> (
              ...
              | Eq -> (
                  ...
                  | Some (Stuck_head (head_head, head_args)) -> (
                      ...
                      let emotive =
                        match motive with
                        | Some (Motive_family t) -> Some (eval_term env t)
                        | Some (Motive_type _) | None -> None in
                      let stored_ty =
                        match motive with
                        | Some (Motive_type t) -> Some (eval_term env t)
                        | Some (Motive_family _) | None -> None in
                      ...
                      | Some match_ty ->
                          let new_branches =
                            Constr.Map.mapi
                              (fun constr br ->
                                match br with
                                (* A refuted branch is rebuilt like an ordinary one, except that its stored term -- the thing of empty type that justified the refutation -- is kinetic, so it is read back synthesizingly and needs no type.  Nothing displays it, since the unparser omits refuted branches; it is rebuilt so that the reconstructed match is a well-formed term. *)
                                | Term.Refute { annotate; comp; perm; tm = rtm } ->
                                    ...
                                    let xs =
                                      ...
                                      | None -> Raw.Indexed.Namevec.none plus_args in
                                    let (Ext_pi
                                           {
                                             ctx = newctx;
                                             values = newvars;
                                      ...
                                      ext_pi ctx window cenv xs (eval_term cenv cty) in
                                    let bodyenv =
                                      take_args env plus_dim newvars window fw annotate comp in
                                    Term.Refute
                                      {
                                        annotate = new_annotate;
                                        comp = new_comp;
                                        perm = id_perm;
                                        tm =
                                          readback_refutation newctx (Permute (perm, bodyenv)) rtm;
                                      }
...
and readback_refutation : type mode m c z a.
    (mode, z, a) Ctx.t -> (mode, m, c) env -> (mode, c) refutation -> (mode, a) refutation =
 fun ctx env (Refutation { window; plus_lock; tm }) ->
  let n = dim_env env in
  let (Has_filter fw) = Modality.filter window n in
  let akenv =
    act_env (key_id_env env plus_lock) (opt_op_of_opt_sface (Modality.sface_of_filter n fw)) in
  let (Locked (new_lock, lctx)) = Ctx.lock ctx window in
  Refutation { window; plus_lock = new_lock; tm = readback_val lctx (eval_term akenv tm) }
...
and higher_codatafield_shuffleable : type mode a b c r h i.
 ...
 fun ctx dbwd src r fldshuf ->
  let nfctx =
    match src with
    | Ambient degenv -> Deg_ctx (ctx, degenv)
    | Display (_, fctx, fdegenv) -> Deg_ctx (fctx, fdegenv) in
  Nontrivial
    {
      dbwd;
      shuffle = fldshuf;
      ...
      deg_nf =
        (fun nf ->
          let (Deg_ctx (ctx, degenv)) = nfctx in
          degenerate_normal ctx degenv r nf);
    }

(* Building the display node for a value-level codatatype or record type, for the "about" command.  This is the codata analogue of readback_data above.  It lives here rather than in Readback because non-projectable higher-field instances are displayed in a context degenerated by their remaining dimensions (Degctx.degctx), and Degctx itself depends on Readback, so Readback cannot depend on Degctx.

   The result is a Codata_display rather than an ordinary Canonical (Codata …), because the instances of a field need not correspond one-to-one to the fields; see the comment on Term.codata_field_display for why a degenerate codatatype with a higher field cannot be represented as a Codata.
...
and readback_codata : type mode a b cm cn ca cet s.
    (mode, a, b) Ctx.t ->
              ...
              (fun acc
                ...
                | Term.Codatafield.Lower (adj, _, _) -> (
                    ...
                    | Some (Self_display { adj; plus_lock; sctx; self_top; self_top_ty; _ }) ->
                        ...
                        Bwd.Snoc (acc, Term.Cfd (adj, fld, [], plus_lock, ty)))
                | Term.Codatafield.Higher (adj, _, _, _, _) -> (
                    match self_display adj with
                    | None -> acc
                      ...
                      ->
                        ...
                        Seq.fold_left
                          (fun acc (Pbij_between (pbij : (cm, i, _) pbij)) ->
                            ...
                            | Zero ->
                                (* Projectable instance: read back in the locked, self-extended context, as for a lower field. *)
                                let Eq = eq_of_zero_shuffle fldshuf in
                                let ety =
                                  tyof_field ~key:`Nokey ~self:`Declared left (Ok self_top)
                                    self_top_ty fld ~shuf:Trivial fldins in
                                Bwd.Snoc
                                  ( acc,
                                    Term.Cfd
                                      ( adj,
                                        ...
                                        plus_lock,
                                        readback_val ~sort:`Type sctx ety ) )
                            | Pos _ ->
                                ...
                                let ety =
                                  tyof_field ~key:`Nokey ~self:`Declared left (Ok self_top)
                                    self_top_ty fld ~shuf fldins in
                                Bwd.Snoc
                                  ( acc,
                                    Term.Cfd_deg
                                      ( adj,
                                        ...
                                        plus_lock,
                                        plusmap,
                                        readback_val ~sort:`Type dsctx ety ) ))
                          acc
                                           ...
                                           Term.Realize
                                             (readback_at Kinetic lctx
                                                ...
                                                (tyof_field left (Ok xu) tyu fld ~shuf:Trivial
                                                   (ins_zero evaldim))),
                                           `Labeled ) ) )))
                    | Some acc, Term.Codatafield.Higher ((Adjunction { left; _ } as adj), _, _, _, _)
                      -> (
                        let (Has_filter lfilter) = Modality.filter left dim in
                        match Modality.filter_is_trivial dim lfilter with
                        | None -> Some acc
