...
let data_display_value : type mode lev e s.
    (mode, lev, e) Ctx.t ->
    bool ->
...
let degenerate_data_display : type mode a b m j ij s.
    (mode, a, b) Ctx.t ->
      ...
      | Val (Value.Canonical { canonical; ins; _ }) -> (
          ...
          | Data data_args -> (
              ...
              | Some _, Zero ->
                  ...
                  Some (data_display_value ctx indexed data_args.constrs)
              | Some _, Pos _ ->
                  (* A degenerate (positive substitution dimension) datatype. *)
                  Some (degenerate_data_display ctx value data_args)
              | None, _ -> None)
