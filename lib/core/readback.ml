      ...
      | Canonical
          ...
          | Eq ->
              ...
              (* The motive and the type of the match itself.  Both are available only in a zero-dimensional environment.
                 ...
                 For the motive: the match we reconstruct is at the total dimension, and the motive of a match at that dimension is a flat family of all the faces of its discriminee whose values are types -- that is what typechecking checks a "return" clause against, and it is what motive_of_family describes.  A degenerated match has no such family: its type at a given discriminee is the degenerated motive instantiated at the matches on that discriminee's faces, so a flat family computing it would have to contain the match itself.  The degeneracy of the stored motive is not one, and nothing else is available to display, so a degenerated match shows no "return" clause. *)
