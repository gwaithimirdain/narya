...
and readback_head : type mode c z.
 ...
 fun ?(sort = (`Other, `Other)) ctx h ->
  match h with
  | Var { level; deg; key } -> (
      let (Remove_lock (ctx, ac)) = Ctx.remove_lock ctx (Modalcell.vcod key) in
      let x = Ctx.find_level ctx level <|> No_such_level (PLevel level) in
      match is_id_deg deg with
      | Some _ -> Key (Var x, ac, key)
      | None -> Key (Act (Var x, deg, sort), ac, key))
  | Const { name; ins } -> (
      let dim = cod_left_ins ins in
 ...
 fun ctx env envctx ->
  ...
  | Ext (envctx, entry, _) -> (
      ...
      | Vis { modality; bindings; _ } | Invis (modality, bindings) ->
          ...
          let lctx = Ctx.lock ctx modality in
          (* We get the top entry (Now) from the environment we're reading back.  We can't just match it against Ext or LazyExt because it could have other lazy operations applied to it like Shift, Unshift, Permute, etc.  We give the identity cell of our current modality as the starting key, since we're looking for a binding annotated with that modality. *)
          let (Looked_up { act; op = Op (fc, fd); entry = xs; key }) =
            lookup_cube env mk Now (id_op (D.plus_out (dim_env env) mk)) idcell in
          let lenv = key_env env idcell in
          ...
          let tmenv = readback_ordered_env ctx (remove_env env Now) envctx in
          Ext (tmenv, mk, modality, tmxs))
  | Lock (envctx, lock, _) ->
      (* We remove enough keys from the environment being read back to make one whose domain matches the lock, then remove enough locks from the context we are reading back *in* to match the codomain of those locks. *)
      let env, Wrap keys = split_env_keys env lock in
      let (Remove_lock (ctx, bc)) = Ctx.remove_lock ctx (Modalcell.vcod keys) in
      Key (readback_ordered_env ctx env envctx, bc, keys)
...
let rec readback_ordered_ctx : type mode a b.
  ...
  | Lock (ctx, lock, parametric) -> Lock (readback_ordered_ctx ctx, lock, parametric)
