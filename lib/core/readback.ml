...
(* The instantiation arguments to assemble into a codatatype's self-variable cube, with the dimensions they contribute existentially quantified (see readback_codata). *)
type (_, _) self_args = Self_args : ('mk, 'j, 'd, 'x) TubeOf.t -> ('mk, 'x) self_args

(* Whether a codatatype has any higher fields, which determines how an instantiation of it is displayed (see readback_codata). *)
let has_higher_field : type mode a m n et. (mode * a * m * n * et) Term.CodatafieldAbwd.t -> bool =
 fun fields ->
  let higher : type i. (i, mode * a * m * n * et) Term.Codatafield.t -> bool = function
    | Lower _ -> false
    | Higher _ -> true in
  Bwd.fold_left (fun acc (Term.CodatafieldAbwd.Entry (_, cf)) -> acc || higher cf) false fields
...
and readback_at : type mode a z s.
 ...
 fun ?(eta = false) status ctx tm ty ->
  ...
  | _, Canonical { canonical; ins; tyargs; _ }, Potential neutral -> (
      ...
      | Codata codata_args -> readback_codata ctx neutral tyargs codata_args
...
(* Raise a normal into a context degenerated by 'r, by reading it back and re-evaluating it in the degenerating environment, instantiating its type at the correspondingly raised faces.  Used to raise the boundary of the self value of a higher codatatype field, whose field projections instantiate the type of a non-projectable instance, and by the comatch readback to raise the self it projects a higher field's instances from. *)
...
(* Raise a whole cube of values into a context degenerated by 'r, in the same way: the value at the (fa,fb) face of the result is the fa-degeneration of the value at the fb face.  No types are involved, so unlike degenerate_normal there is nothing to instantiate.  Used to raise the self value of a higher codatatype field, along with its boundary, into the context where a non-projectable instance of that field lives. *)
...
(* Build the "nontrivial shuffleable" passed to tyof_higher_codatafield when computing the type of a non-projectable instance of a higher codatatype field.  The remaining dimensions 'r have been split off, and the field's shuffle is fldshuf.  Its callback eval-readbacks the field type's closure environment to move it into the context degenerated by those dimensions.

   That environment contains only the codatatype's parameters -- the self variable is supplied to the field type after this degeneration, not carried through it -- so its values always live in the ambient context locked by the field's right adjoint, whether we are checking a field against an ambient self value or displaying a codatatype whose self is a variable of its own.  Hence the single implementation here, and hence all we need from the caller is the degeneration of the ambient context. *)
and higher_codatafield_shuffleable : type mode a b r h i.
    (mode, a, b) Ctx.t -> (mode, r, b) env -> (r, h, i) shuffle -> (mode, r, h, i) shuffleable =
 fun ctx degenv fldshuf ->
  Nontrivial
    {
      shuffle = fldshuf;
      deg_env =
...
(* Read back a codatatype or record type.  Non-projectable higher-field instances are displayed in a context degenerated by their remaining dimensions (Degctx.degctx), which is why this file has a forward reference to Degctx (see the top of the file).
   ...
   The boundary of a degenerate codatatype behaves like *parameters*, not like the indices that the boundary of a degenerate datatype behaves like: it is the *instantiated* form √A⁽ᵉ⁾ s0 s1 that one defines elements of by comatching, not √A⁽ᵉ⁾ itself.  So a codatatype value that has been instantiated is displayed as it is, showing the types of the fields such a comatch must supply, rather than being read back uninstantiated and re-instantiated the way readback_data does for a datatype.  For that, the self-variable cube is assembled from the instantiation arguments, which are its boundary faces exactly as they are for the self value of a comatch being checked, together with a fresh variable cube for the uninstantiated dimensions on top.  Its dimension is thus the codatatype's whole dimension even when the instantiation has lowered the dimension of the self *variable* itself, and the field instances are indexed by partial bijections from that same whole dimension, which is what a comatch must supply.

   We do that only when the codatatype has a higher field, though, since only there does the difference show up: with lower fields alone the instances are indexed by nothing, and displaying an instantiated record type with a self variable of the lowered dimension lets it come out in the nicer "sig" field-variable syntax. *)
and readback_codata : type mode a b cm cn ca cet iu ii iout.
 ...
 fun ctx tm insargs codata_args ->
  ...
  | Neu { ty; _ } -> (
      ...
      | Canonical
          ...
          (* The uninstantiated part of the value's dimension is the dimension of the universe it belongs to, which is where the fresh part of the self-variable cube lives. *)
          let (Self_args (type j d) (insargs : (mn, j, d, mode normal) TubeOf.t)) =
            (match (has_higher_field codata_args.fields, D.compare (TubeOf.uninst insargs) mk) with
             | true, Eq -> Self_args insargs
             | true, Neq ->
                 fatal
                   (Anomaly "uninstantiated dimension of a codatatype is not that of its universe")
             (* With no higher fields we ignore the instantiation, keeping the self variable at the lowered dimension. *)
             | false, _ -> Self_args (TubeOf.empty mk)
              : (mn, mode normal) self_args) in
          ...
          let module Self_display = struct
            (* This is parametrized by the field's adjunction rather than existentially quantifying over it, so that the locked context here is the same one that the field's own stored data lives over. *)
            type (_, _, _, _, _, _) t =
              | Self_display : {
                  plus_lock : ('b, 'mode, 'g, 'gmode, 'bg) plus_lock;
                  lctx : ('gmode, 'raw, 'bg) Ctx.t;
                  sctx : ('gmode, 'sraw, ('bg, ('f, 'n) dim_entry) snoc) Ctx.t;
                  (* The self variable, with its *untransported* types. *)
                  ambnfs : ('n, 'mode Ctx.Binding.t) CubeOf.t;
                }
                  -> ('mode, 'b, 'n, 'f, 'g, 'gmode) t
              | No_display : ('mode, 'b, 'n, 'f, 'g, 'gmode) t
          end in
          let self_display : type f g gmode.
              (mode, f, g, gmode) Modalcell.adjunction -> (mode, b, d, f, g, gmode) Self_display.t =
           fun (Adjunction { left; right; unit; _ }) ->
            let (Locked (plus_lock, lctx)) = Ctx.lock ctx right in
          ...
          let fields =
            Bwd.fold_left
              (fun (acc : (mode * b * d * d * cet) Term.CodatafieldAbwd.t)
                    ...
                    | Self_display { plus_lock; sctx; ambnfs; _ } ->
                        ...
                        let ety =
                          tyof_field ~key:`Nokey ~self:`Declared (Modalcell.adj_left adj)
                            (Ok self.tm) (Lazy.force self.ty) fld (ins_zero evaldim) in
                ...
                | Term.Codatafield.Higher (adj, fld_plus_lock, fldtermctx, fldtys) -> (
                    ...
                    | Self_display { plus_lock; lctx; sctx; ambnfs } -> (
                        ...
                        | Eq ->
                            let tys =
                              readback_higher_codatafield ctx codata_args.env fld_plus_lock
                                fldtermctx fldtys (CubeOf.dim ambnfs) fld adj ambnfs sctx in
                            ...
                            Bwd.Snoc
                              ( acc,
                                Term.CodatafieldAbwd.Entry
                                  ( fld,
                                    Term.Codatafield.Higher (adj, plus_lock, readback_ctx lctx, tys)
                                  ) ))))
...
(* Read back the instances of one higher field of a codatatype value being displayed, as the pbijmap of types that Codatafield.Higher stores: one entry per partial bijection between the codatatype's dimension m and the field's intrinsic dimension.  The self variable, its cube, and the context they live in are those built by readback_codata's self_display, while the environment, lock, termctx and stored types are those of the codatatype value being displayed; the ambient context is needed only to degenerate for a non-projectable instance. *)
      ...
      build =
        ...
        ->
          ...
          | Pos _ ->
              ...
              let shuf = higher_codatafield_shuffleable ctx degenv fldshuf in
