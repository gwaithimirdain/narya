 ...
 fun env tm ->
  ...
  | Match { tm; window; plus_lock; dim = match_dim; branches; motive = _ } as match_tm -> (
      ...
      | Constr (name, constr_dim, dargs) -> (
          ...
          | Some (Refute _) -> Unrealized (Some (Potential_neu (Stuck (env, match_tm), Emp))))
...
and tyof_codatafield : type src f mode m n mn a k r s i et.
    ...
    (m, s, k) insertion ->
    (mode, kinetic) value =
 fun ?(key = `Counit) ?(self = `Ambient) fm tm fldname fldty env tyargs m mn ~shuf fldins ->
  ...
  | Term.Codatafield.Lower (adj, plus_lock, fldty) -> (
      ...
      | Eq -> tyof_lower_codatafield tm fldname adj plus_lock fldty env tyargs m mn ~key ~self)
...
and tyof_higher_codatafield : type mode f g gmode c n h s r i d ag iagx.
    ...
    (* As for lower fields, ~key:`Counit keys the result by the adjunction counit (for a projection) while ~key:`Nokey leaves it behind the g-lock (for checking/reading back a tuple component), and ~self says whether the supplied self value is ambient or is already the declared self variable. *)
    key:[ `Counit | `Nokey ] ->
    self:[ `Ambient | `Declared ] ->
    (* In the nontrivial case, the return value is also in the degenerated context. *)
    (gmode, kinetic) value =
 fun tm fldname adj codataenv tyargs fldins ~shuf plus_lock fldtermctx ic0 fldty ~key ~self ->
...
and tyof_field : type src f mode m h s r i c.
    ...
    (m, s, h) insertion ->
    (mode, kinetic) value =
 fun ?(key = `Counit) ?(self = `Ambient) fm tm ty fld ~shuf fldins ->
  let errtm =
    match tm with
    | Ok tm -> Dump.Val tm
  ...
  | Canonical
      ...
      | Some mn ->
          tyof_field_giventype ~key ~self fm tm head eta env mn fields tyargs fld ~shuf fldins)
...
and tyof_field_giventype : type src f mode m n mn h s r i c et a k hmode.
    ...
    (k, s, h) insertion ->
    (mode, kinetic) value =
 fun ?(key = `Counit) ?(self = `Ambient) fm tm head eta env mn fields tyargs fld ~shuf fldins ->
  let severity = Asai.Diagnostic.Bug in
  let errfld =
    match shuf with
    | Trivial -> `Ins (fld, fldins)
  ...
  | Eq -> (
      ...
      | Found fldty ->
          ...
          tyof_codatafield ~key ~self fm tm fld fldty env tyargs m mn ~shuf fldins
...
and eval_codata : type mode m a n et.
 ...
 fun env eta opacity hints n fields fibrancy_fields ->
  ...
  let canonical = Codata { eta; opacity; hints; env; fields } in
...
(* ******************** Walking constructor function-types ******************** *)

(* Datatype constructors store their function-types as terms; to check, compare, or read back a constructor application we evaluate that function-type and walk the resulting pi-type values, one argument at a time.  This view extracts the pi-type data from such a value, which is always an uninstantiated pi-type of the dimension of the constructor application. *)
type (_, _) viewed_pi =
  | Viewed_pi : ('dom, 'modality, 'mode, 'k, 'n) Value.pi_args -> ('mode, 'n) viewed_pi

let view_constr_pi : type mode n. string -> n D.t -> (mode, kinetic) value -> (mode, n) viewed_pi =
 fun err n ty ->
...
let constr_boundary_tube : type dom modality mode k n.
    string ->
    n D.t ->
    (dom, modality, mode, k, n) Modality.filter_dim ->
    (k, (dom, kinetic) value) CubeOf.t ->
