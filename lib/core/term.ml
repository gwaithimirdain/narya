...
module rec Term : sig
  ...
  and (_, _) canonical =
    (* A datatype stores its family of constructors, whether it has recursive constructors, and also its number of indices.  (The former two are not determined in the latter if there happen to be zero constructors). *)
    | Data : {
        indices : 'i Fwn.t;
        constrs : (Constr.t, ('mode, 'a, 'i) dataconstr) Abwd.t;
        recursive : Positivity.recursion;
