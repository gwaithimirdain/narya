...
module rec Term : sig
  ...
  and _ canonical_display =
    ...
    | Codata_display : {
        eta : (potential, 'et) eta;
        dim : 'n D.t;
        fields : ('a, 'n) codata_field_display Bwd.t;
      }
        -> 'a canonical_display
  ...
  and (_, _) codata_field_display =
    (* A field instance displayed in the self-extended base context: the field, its field-application suffix (e.g. ["1"]/["2"] for projectable higher fields, [] for lower fields), and the field's type referring to the self-variable. *)
    | Cfd :
        'i Field.t * string list * (('a, 'n) snoc, kinetic) term
        -> ('a, 'n) codata_field_display
    (* A non-projectable higher field instance, displayed in a context degenerated by the remaining dimension 'r: the field, its suffix, that dimension, the degeneration plus-map from the self-extended base context, and the field's type there. *)
    | Cfd_deg :
        'i Field.t * string list * 'r D.t * ('r, ('a, 'n) snoc, 'kb) Plusmap.t * ('kb, kinetic) term
        -> ('a, 'n) codata_field_display
