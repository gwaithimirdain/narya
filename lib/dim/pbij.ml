...
let rec pbij_of_int_strings : type e.
 ...
 fun e strs ->
  ...
  | `Str str :: strs when str = Endpoints.refl_string () -> (
      ...
      (* A remaining dimension has to move past all the shared ones, so this only denotes a partial bijection if it commutes with them. *)
      | Some (Pbij_of (Pbij (ins, shuf))) -> (
          match D.gen_commute D.deg (right_shuffle shuf) with
          | Some c -> Some (Pbij_of (Pbij (ins, Left (D.deg, c, shuf))))
          | None -> None)
      | None -> None)
  | `Str _ :: _ -> None
...
let rec int_strings_of_pbij : type n i r. (n, i, r) pbij -> [ `Int of int | `Str of string ] list =
 fun (Pbij (ins, shuf)) ->
  match shuf with
  | Zero -> []
  | Left (_, _, shuf) -> `Str (Endpoints.refl_string ()) :: int_strings_of_pbij (Pbij (ins, shuf))
  | Right (_, shuf) ->
...
let rec deg_comp_ins : type m n i res.
 ...
 fun ic deg ins ->
  match (ins, ic) with
  | Zero _, _ -> Deg_comp_ins (Zero (cod_deg deg), Zero, deg)
  | Suc (ins, g, i), Central_suc (ic, _, gc) -> (
      match deg_coresidual g deg i with
      | Coresidual_zero deg ->
          let (Deg_comp_ins (ins, shuf, s)) = deg_comp_ins ic deg ins in
          (* This generator is degenerated by the degeneracy, so it moves from the shared part to the remaining part, and hence has to pass all the shared generators that survive; being an intrinsic generator, it is central. *)
          Deg_comp_ins (ins, Left (g, D.gen_commute_central g gc (right_shuffle shuf), shuf), s)
...
let rec deg_comp_pbij : type m n i res rem sh.
 ...
 fun ic deg ins shuf ->
  ...
  | Left (g, gsh, shuf) ->
      let (Central_suc (ic, _, _)) = ic in
      let (Deg_comp_pbij (ins, shuf, s, sub, _)) = deg_comp_pbij ic deg ins shuf in
      Deg_comp_pbij
        ( ins,
  ...
  | Right (g, shuf) -> (
      ...
      | Coresidual_zero deg ->
          ...
          Deg_comp_pbij
            ( ins,
              Left (g, D.gen_commute_central g gc (right_shuffle shuf), shuf),
              s,
              Sub_drop (sub, g),
              function
              | _ -> . )
      | Coresidual_suc (deg, j) ->
          let (Deg_comp_pbij (ins, shuf, s, sub, ifzero)) = deg_comp_pbij ic deg ins shuf in
...
(* The remaining dimensions, being intrinsic ones, are central, which is what it takes to move them past the evaluation dimension. *)
let ins_plus_of_pbij : type n s h r i rn.
    ...
    (rn, s, i) insertion =
 fun ic ins shuf rn ->
  go_ins_plus_of_pbij (D.commute_central (left_central shuf ic) (D.plus_right rn)) ins shuf rn
...
let rec plus_comp_ins : type e1 e2 e12 res1 res2 sh1 sh2 sh12.
 ...
 fun sc e12 ins1 ins2 shuf ->
  ...
  | Left (g, _, shuf) ->
      let (Central_suc (sc, _, gc)) = sc in
      let (Suc (ins1, _, x1)) = ins1 in
      (* The generator inserted into 'e1 has to move past the whole of 'e2 to reach the outer end of 'e12, which again follows from the intrinsic dimensions being central. *)
      let (Insert_plus (e12', y)) =
        D.insert_plus (D.gen_commute_central g gc (dom_ins ins2)) x1 e12 in
      let (Plus_comp_ins ins12) = plus_comp_ins sc e12' ins1 ins2 shuf in
      Plus_comp_ins (Suc (ins12, g, y))
  (* Or it comes from 'sh2, hence is inserted into the 'e2 part of 'e12. *)
  | Right (g, shuf) ->
      let (Central_suc (sc, _, _)) = sc in
...
let plus_comp_pbij : type e1 e2 e12 i r2 r12.
    ...
    (e1, r2, r12) pbij ->
    (e12, i, r12) pbij =
 fun ic e12 (Pbij (ins2, shuf2)) (Pbij (ins1, shuf1)) ->
  ...
  (* And that same shuffle tells us how to combine the two insertions.  The shared dimensions, being part of the intrinsic ones, are central. *)
  let (Plus_comp_ins ins12) = plus_comp_ins (right_central shuf12 ic) e12 ins1 ins2 sh12 in
  Pbij (ins12, shuf12)
...
functor
  ...
  struct
    ...
    type (_, _, _, _) gt =
      ...
      | Suc : {
          g : 'g D.G.t;
          (* The keys of this map are all the ways of matching the intrinsic dimensions with the evaluation ones, so each intrinsic generator has to be able to go anywhere: we store the witness that it is central. *)
          central : 'g D.G.central;
          left : ('evaluation, 'intrinsic, ('g, 's) cons, 'v) gt;
          right : ('evaluation, 'g, ('intrinsic * 's) * 'v) Tup.t;
        }
...
module Pbijmap (F : Fam2) = struct
  ...
  (* And it is automatically central, since that is what it took to build the map. *)
  let rec gcentral : type evaluation intrinsic r v.
      (evaluation, intrinsic, r, v) gt -> intrinsic D.central = function
    | Zero _ -> Central_zero
    | Suc { g; central; left; _ } -> Central_suc (gcentral left, g, central)

  let central : type evaluation intrinsic v. (evaluation, intrinsic, v) t -> intrinsic D.central =
   fun (ms, _) -> gcentral ms
  ...
  let rec gfind : type evaluation intrinsic s r2 r v.
   ...
   fun p m r12 ->
    ...
    | Pbij (Zero _, Zero), Zero (bp, v) ->
        ...
        v
    | Pbij (ins, Left (_, _, shuf)), Suc m -> gfind (Pbij (ins, shuf)) m.left (Append_cons r12)
  ...
  (* Since the intrinsic dimension has to be central, we take its centrality witness as the argument, from which the dimension itself can be read off. *)
  let rec gbuild : type evaluation intrinsic s v.
   ...
   fun evaluation intrinsic f ->
    ...
    | Central_suc
        (type i1 g0t)
        ((intrinsic, g_intrinsic, central) : i1 D.central * g0t D.G.t * g0t D.G.central) ->
        Suc
          {
            g = g_intrinsic;
            central;
                      ...
                      f.build
                        (Pbij
                           ( ins,
                             Left
                               ( g_intrinsic,
                                 D.gen_commute_central g_intrinsic central (right_shuffle shuf),
                                 shuf ) ))
                        r12');
                };
            right =
              (let build : type b. (b, g0t, evaluation) D.insert -> (b, (i1 * s) * v) Param.t =
                fun i ->
                 Sub
                   (gbuild (D.uninsert i evaluation) intrinsic
                      {
                        f with
                        build =
                          (fun (Pbij (ins, shuf)) r12 ->
               ...
               Tup.build evaluation g_intrinsic
                 (D.gen_commute_central g_intrinsic central evaluation)
                 { build });
          }
  ...
  module Heter = struct
    ...
    let rec suc : type e i s g vs irvs.
     ...
     fun g central ls irvs rs ->
      match (ls, irvs, rs) with
      | [], [], [] -> []
      | left :: ls, Times :: irvs, right :: rs ->
          Suc { g; central; left; right } :: suc g central ls irvs rs
  ...
  let rec gpmap : type evaluation intrinsic remaining v vs ws.
   ...
   fun evaluation f ms ws ->
    ...
    | Zero _ :: _ ->
        ...
        Heter.zero bp res
    | Suc { g = g_outer; central; _ } :: _ -> gpmap_suc evaluation g_outer central f ms ws
  ...
  and gpmap_suc : type evaluation i1 g0t s v vs ws.
   ...
   fun evaluation g_outer central f ms ws ->
          ...
          map =
            (fun (Pbij (ins, shuf)) r12 v ->
              ...
              f.map
                (Pbij
                   ( ins,
                     Left (g_outer, D.gen_commute_central g_outer central (right_shuffle shuf), shuf)
                   ))
                r12' v);
        }
        (Heter.left ms) ws in
    let rights = Tup.pmap { map } (Heter.right ms irvs) (MapTimes.cod irws) in
    Heter.suc g_outer central lefts irws rights
