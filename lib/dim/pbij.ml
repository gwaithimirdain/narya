 ...
 fun deg ins ->
  ...
  | Suc (ins, g, i) -> (
      ...
      | Coresidual_zero deg ->
          ...
          (* This generator is degenerated by the degeneracy, so it moves from the shared part to the remaining part, and hence has to pass all the shared generators that survive.  Although insertions in general don't index higher fields, this function is called only from act_higher_structfield, on insertions whose intrinsic dimension is that of a higher field, so we may ask for centrality. *)
          Deg_comp_ins
            (ins, Left (g, D.gen_commute_central g (D.free_central g) (right_shuffle shuf), shuf), s)
 ...
 fun deg ins shuf ->
  ...
  | Right (g, shuf) -> (
      ...
      | Coresidual_zero deg ->
          ...
          Deg_comp_pbij
            ( ins,
              Left (g, D.gen_commute_central g (D.free_central g) (right_shuffle shuf), shuf),
              s,
              Sub_drop (sub, g),
              function
              | _ -> . )
...
let ins_plus_of_pbij : type n s h r i rn.
 ...
 fun ins shuf rn ->
  go_ins_plus_of_pbij
    (D.commute_central (D.free_word_central (left_shuffle shuf)) (D.plus_right rn))
    ins shuf rn
 ...
 fun e12 ins1 ins2 shuf ->
  ...
  | Left (g, _, shuf) ->
      ...
      let (Insert_plus (e12', y)) =
        D.insert_plus (D.gen_commute_central g (D.free_central g) (dom_ins ins2)) x1 e12 in
...
module Pbijmap (F : Fam2) = struct
  ...
  let rec gbuild : type evaluation intrinsic s v.
   ...
   fun evaluation intrinsic f ->
    ...
    | Word (Suc (type i1 g0t) ((intrinsic, g_intrinsic) : (_, i1, _) D.plus * g0t D.G.t)) ->
        (* The keys of this map are the partial bijections between the intrinsic and evaluation dimensions, which exist for every matching only if this intrinsic generator is central. *)
        let central = D.free_central g_intrinsic in
        Suc
          {
            g = g_intrinsic;
            left =
  ...
  and gpmap_suc : type evaluation i1 g0t s v vs ws.
   ...
   fun evaluation g_outer f ms ws ->
    ...
    (* As in gbuild, this map exists only if its outer intrinsic generator is central. *)
    let central = D.free_central g_outer in
    let lefts =
      gpmap evaluation
        {
