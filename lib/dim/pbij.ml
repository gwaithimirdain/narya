 ...
 fun deg ins ->
  ...
  | Suc (ins, g, i) -> (
      ...
      | Coresidual_zero deg ->
          ...
          (* This generator is degenerated by the degeneracy, so it moves from the shared part to the remaining part, and hence has to pass all the shared generators that survive.  Unlike the operations on partial bijections below, this one is also called on insertions that don't index higher fields, so we ask only for commutation rather than centrality. *)
          Deg_comp_ins (ins, Left (g, D.free_gen_commute (right_shuffle shuf), shuf), s)
