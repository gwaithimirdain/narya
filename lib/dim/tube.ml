...
module Tube (F : Fam2) = struct
  ...
  type (_, _, _, _, _) gt =
    ...
    | Branch :
        ...
        * 'l Endpoints.len
        * (('mk, 'n, 'b) C.gt, 'l) Bwv.t
        * ('m, 'k, 'mk, ('n, 'g) D.suc, 'b) gt
        -> ('m, ('k, 'g) D.suc, ('mk, 'g) D.suc, ('n, 'g) D.suc, 'b) gt
  ...
  *)
  ...
  let rec gpboundary : type m k mk l kl mkl n b.
   ...
   fun mk kl tr ->
    ...
    | Suc (kl, Unit), Branch (_, l, ends, mid) ->
        Branch (D.deg, l, ends, gpboundary mk kl mid)
    ...
    let rec branch : type m k mk n bs hs l.
     ...
     fun l hs endss mids ->
      ...
      | Cons hs, ends :: endss, mid :: mids ->
          Branch (D.deg, l, ends, mid) :: branch l hs endss mids
  end
                ...
                (M.zip
                   (fun () ->
                     BwvM.pmapM
                       (fun (e :: brs) ->
                         M.apply
                           (gpmapM_ll mk'' ml' ml1' (LEnd (D.deg, e, d)) g
    ...
    let rec gbuildM_ll : type k m mk l1 l2 l ml ml1 b.
     ...
     fun m mk ml ml1 d g ->
      ...
      | Word (Suc (m, Unit)) ->
          ...
          @@ fun (ends, mid) -> C.Branch (D.deg, l, ends, mid)
