...
module Make (G : Word.Gen) (F : Fam2) = struct
  ...
  let rec gset : type a asuc g b ab p.
   ...
   fun i v m ab ->
    ...
    | Later i ->
        let (Map m) = m in
        Map { m with later = gset i v m.later (Suc ab) }
  ...
  let rec gupdate : type a asuc g b ab p.
   ...
   fun i f m ab ->
    ...
    | Later i ->
        let (Map m) = m in
        Map { m with later = gupdate i f m.later (Suc ab) }
  ...
  let rec gbuild : type a b ab p. a W.t -> (a, b, ab) W.bplus -> (ab, p) builder -> (a, b, p) gt =
   fun a ab builder ->
    ...
    | Word Zero -> Emp
    | Word (Suc (a', g)) ->
        let (Bplus bplus) = W.bplus (W.bplus_right ab) in
        Map
          {
            gen = g;
            bplus;
            now = builder.build g (W.insert_bplus Now bplus ab);
            later = gbuild (Word a') (Suc ab) builder;
          }

  let build : type a p. a W.t -> (a, p) builder -> (a, p) t = fun a builder -> gbuild a Zero builder
      ...
      | Map { gen; bplus = ab'; now = v; later } :: mss ->
          M.apply
            (M.zip
               ...
               (fun () -> gpmapM (Suc ab) f (later :: Heter.later mss) qs))
