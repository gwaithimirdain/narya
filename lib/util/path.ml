...
module Map
    ...
    (QM : MAP3_MAKER with module Key := Q) :
  MAP3_MAKER with module Key := Make(Q) = struct
  module Cat = Make (Q)
  ...
  module Make (F : Fam4) = struct
    ...
    let find_opt : type p src m tgt.
        (src, m, tgt) Cat.t -> p t -> (p, src, m, tgt) F.t option =
     fun path t ->
      let open Monad.Ops (Monad.Maybe) in
    ...
    let add : type p src m tgt.
        (src, m, tgt) Cat.t -> (p, src, m, tgt) F.t -> p t -> p t =
     fun path value t ->
      let (I.To_fwd (fwd, bcomp)) = I.to_fwd path in
    ...
    let remove : type p src m tgt. (src, m, tgt) Cat.t -> p t -> p t =
     fun path t ->
      ...
      ObjMap.update (Cat.tgt path)
        (Option.map (fun path_map -> I.remove fwd path_map))
        t
    ...
    let map : type p. p mapper -> p t -> p t =
     fun f t ->
      ObjMap.map
        { map = (fun tgt_obj path_map -> I.map f (Cat.id tgt_obj) path_map) }
        t
    ...
    let iter : type p. p iterator -> p t -> unit =
     fun f t ->
      ObjMap.iter
        { it = (fun tgt_obj path_map -> I.iter f (Cat.id tgt_obj) path_map) }
        t
  end
end
