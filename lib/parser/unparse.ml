...
and unparse_comatch : type mode n a s et lt ls rt rs.
 ...
 fun vars _dim fields _li _ri ->
  ...
  (* TODO(about-merge): displaying a *higher* field of a comatch is deferred.  Name degeneration (Names.degenerate) is now available, but the *readback* side (Canonical_display.readback_comatch_impl) still falls back rather than producing the higher Structfield from modal's PlusPbijmap/PlusFam, so this branch is never reached; it remains a stub until that readback port is done. *)
  let higher_fields : type i.
      observation Bwd.t -> i Field.t -> (n, i, mode * a) Term.PlusPbijmap.t -> observation Bwd.t =
   fun _acc _fld _pbijmap -> fatal (Unimplemented "displaying higher comatch fields (post-modal-merge)") in
  match fields with
  | Emp ->
      unlocated
