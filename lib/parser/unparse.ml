...
let rec unparse : type mode n lt ls rt rs s.
 ...
 fun vars tm li ri ->
  ...
  | Var x -> (
      ...
      | `Name x -> unlocated (Ident (x, []))
      (* A variable bound to an alternative unparser -- the self-type variable of a datatype -- displays as whatever that unparser produces here. *)
      | `Other unp -> unp.unparse li ri)
  | Const c -> (
  ...
  | Unapply tm -> unparse vars tm li ri
...
and make_unparser : type mode n s. (unparser, n) Names.t -> (mode, n, s) term -> unparser =
 fun vars tm -> { unparse = (fun li ri -> unparse vars tm li ri) }

(* A version that wraps implicit arguments in braces. *)
and make_unparser_implicit : type n. (unparser, n) Names.t -> (n, kinetic) spine_arg -> unparser =
...
and unparse_codata : type mode m n a et lt ls rt rs.
 ...
 fun vars eta evaldim selfdim fields _li _ri ->
  ...
  let module Self = struct
    type t = {
      ext : 'b 'k. (unparser, 'b) Names.t -> (unparser, ('b, ('k, n) dim_entry) snoc) Names.t;
    }
  end in
...
and unparse_field_var : type mode n lt ls rt rs s.
    (unparser, n) Names.t -> (mode, n, s) term -> string -> (lt, ls, rt, rs) parse located option =
 fun vars tm fld ->
  match tm with
  | Var x -> (
...
and unparse_named_inst : type mode n lt ls rt rs m k mk s.
    (unparser, n) Names.t ->
    (mode, n, s) term ->
    (m, k, mk, (unparser, mode) Names.named_term) TubeOf.t ->
    (lt, ls) No.iinterval ->
    (rt, rs) No.iinterval ->
    (lt, ls, rt, rs) parse located =
...
and unparse_higher_pi : type dom modality mode a am lt ls rt rs k n.
    ...
    (n, dom * modality * mode * a) CodCube.t ->
    (D.zero, n, n, (unparser, mode) Names.named_term) TubeOf.t ->
    (lt, ls) No.iinterval ->
    (rt, rs) No.iinterval ->
    (lt, ls, rt, rs) parse located =
 fun vars accum xs filter plus doms cods tyargs li ri ->
  ...
  let tyargs =
    let map : type kk.
        (kk, D.zero, n, n) tface ->
        (unparser, mode) Names.named_term ->
        (unparser, mode) Names.named_term =
     fun s (Names.Named (type b) ((lamvars, lam) : (unparser, b) Names.t * (mode, b, kinetic) term))
       ->
      ...
      | Filter_sface
          ...
          let _, (lamvars : (unparser, (b, (modality, ks) dim_entry) snoc) Names.t) =
...
let rec unparse_ctx : type dom modality mode a b.
    ...
    (unparser, b) Names.t * Print.printed_entry Bwd.t =
