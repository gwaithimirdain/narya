...
and unparse_codata : type mode m n a et lt ls rt rs.
 ...
 fun vars eta evaldim selfdim fields _li _ri ->
  ...
  let self_var_render () =
    ...
    let x, _ = Names.add_cube selfdim vars (`Anon no_hints) in
    let self = { Self.ext = (fun v -> snd (Names.add_cube selfdim v (`Anon no_hints))) } in
    match eta with
    | Noeta ->
        let inner =
          Bwd.fold_left
            (fun acc { Instance.adj = Any_adjunction adj; name; suffix; ty } ->
    ...
    | Eta ->
        let inner, _ =
          Bwd.fold_left
            (fun (acc, first) { Instance.adj = Any_adjunction adj; name; suffix; ty } ->
              let pat = unparse_field_decl x adj name suffix in
...
(* Unparse a match "match tm [ | constr. x ... |-> body | ... ]".  *)
