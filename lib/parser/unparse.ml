...
and unparse_codata : type mode n a et lt ls rt rs.
 ...
 fun vars eta _dim fields _li _ri ->
  (* Unparse the type of a single field, in a names-context that exposes the self-variable (and, for records, its fields), returning also the field pattern's degeneracy suffix (empty for a lower field, e.g. ".e" for a higher field of dimension 1).  A higher field's declaration type is stored in a context degenerated by the field's full intrinsic dimension (the ".e…e" declaration form); we reconstruct the degenerated variable names with Names.degenerate, and its suffix is that of the "all-remaining" partial bijection (a higher field forces a zero-dimensional codatatype, so this pbij is unique). *)
  let field_ty : type i.
   ...
   fun selfvars fld cf ->
    ...
    | Term.Codatafield.Lower (_, plus_lock, tm) ->
        (* The field type lives behind the lock by the right adjoint (trivial for an ordinary non-modal field), so we expose that lock in the names-context before unparsing. *)
        ([], unparse (Names.add_lock selfvars plus_lock) tm No.Interval.entire No.Interval.entire)
    | Term.Codatafield.Higher (_, plusmap, plus_lock, cty) ->
        let dnames = Names.degenerate (Field.dim fld) plusmap selfvars in
        let suffix = strings_of_pbij (Pbij (ins_zero D.zero, shuffle_zero (Field.dim fld))) in
        (suffix, unparse (Names.add_lock dnames plus_lock) cty No.Interval.entire No.Interval.entire)
  in
        ...
        let inner =
          Bwd.fold_left
            (fun acc (Term.CodatafieldAbwd.Entry (fld, cf)) ->
              let suffix, ty = field_ty selfvars fld cf in
              let pat = unparse_field_app x (Field.to_string fld) suffix in
        ...
        let inner, _ =
          Bwd.fold_left
            (fun (acc, first) (Term.CodatafieldAbwd.Entry (fld, cf)) ->
              let _, ty = field_ty selfvars fld cf in
      ...
      Bwd.fold_left
        (fun acc entry ->
          ...
          | Term.Cfd_deg (fld, suffix, r, plusmap, tm) ->
              ...
              acc
              <: display_field suffix fld (unparse dnames tm No.Interval.entire No.Interval.entire))
        Emp fields in
    match eta with
    | Noeta ->
        let inner =
