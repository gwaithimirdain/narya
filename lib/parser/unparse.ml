...
and unparse_codata : type n a et lt ls rt rs.
 ...
 fun vars eta _dim fields _li _ri ->
  ...
  | Eta -> (
      (* Try the field-variable syntax "sig (a : ..., b : a → ..., ...)", exposing the anonymous self-variable's fields as named variables.  If a field type references the self-variable other than via a field projection, "Names.lookup" raises Self_used and we fall back to the self-variable syntax. *)
      try
        let field_names =
          Bwd.fold_left
      ...
      with Names.Self_used -> self_var_render ())
...
and unparse_canonical_display : type a lt ls rt rs.
 ...
 fun vars cd _li _ri ->
  ...
  | Codata_display { eta; dim = mk; fields } -> (
      ...
      | Noeta -> self_var_render ()
      | Eta -> (
          (* A record type has only (projectable, lower) fields, displayed with the field-variable syntax "sig (a : ..., b : a → ..., ...)": we introduce an anonymous self-variable with its fields exposed as named variables, so that the field types' projections of the self read back as field variables.  A direct use of the self instead raises Self_used. *)
          try
            let field_names =
              Bwd.fold_left
                (fun acc entry ->
...
and unparse_comatch : type n a s et lt ls rt rs.
 ...
 fun vars dim fields _li _ri ->
  (* Render the instances of a higher field: one per partial bijection between the comatch's dimension and the field's intrinsic dimension, exactly as the codatatype declaration lists them.  Each body was checked in a context degenerated by the partial bijection's remaining dimensions, recorded by the plus-map stored alongside it, so we degenerate the names to match before unparsing it (cf. the codata-declaration display). *)
  let higher_fields : type i.
      observation Bwd.t -> i Field.t -> (n, i, a) Term.PlusPbijmap.t -> observation Bwd.t =
   fun acc fld pbijmap ->
    Seq.fold_left
