...
type 'b ctx =
  | Emp : emp ctx
  | Snoc : 'b ctx * ('n, string) gvariables * (string, string) Abwd.t -> ('b, 'n) snoc ctx
...
let add_cube : type n b. n D.t -> b t -> binder_name -> string * (b, n) snoc t =
 fun n { ctx; used } name ->
...
let add_fields : type b n. n D.t -> b t -> string list -> ((b, n) snoc t * string list) option =
...
let add : type b n. b t -> n variables -> (n, string) gvariables * (b, n) snoc t =
...
let add_strings : type b n. b t -> (n, string) gvariables -> (n, string) gvariables * (b, n) snoc t
    =
 fun { ctx; used } (Variables (m, mn, names)) ->
...
let add_full : type b mn. b t -> mn variables -> (mn, string) gvariables * (b, mn) snoc t =
