...
(* We store a parametrized list of cubes of variable names like a context, along with a mapping of field names to local variable names bound textually to that field of the variable.  Unlike in terms, every variable stored here has a name: anonymous variables are assigned an autogenerated name when they are added.
   ...
   An entry can also be an "other": not a name at all, but a client-supplied payload of the parameter type 'x, which "lookup" returns instead of a name.  The unparser uses this for a variable that is to be displayed as something other than an identifier -- the self-type variable of a datatype, which displays as the datatype's own type family.  Uniquifying does not apply to such an entry, there being no string to make unique, and it exposes no fields. *)
type (_, _) ctx =
  | Emp : ('x, 'mode emp) ctx
  | Snoc :
      ('x, 'b) ctx * ('n, string) gvariables * (string, string) Abwd.t
      -> ('x, ('b, ('modality, 'n) dim_entry) snoc) ctx
  | Other : ('x, 'b) ctx * 'x -> ('x, ('b, ('modality, 'n) dim_entry) snoc) ctx
  | Lock : ('x, 'b) ctx -> ('x, ('b, 'modality lock_entry) snoc) ctx

(* We also store a set of which variables already exist, so that we can be sure of creating a new unused one. *)
type ('x, 'b) t = { ctx : ('x, 'b) ctx; used : StringSet.t }
type 'x wrapped = Wrap : ('x, 'n) t -> 'x wrapped

(* An entry of a name-context, as extracted and reinserted by permutation. *)
type (_, _) entry =
  | Entry_vars : ('n, string) gvariables * (string, string) Abwd.t -> ('x, 'n) entry
  | Entry_other : 'x -> ('x, 'n) entry

let empty : type x mode. (x, mode emp) t = { ctx = Emp; used = StringSet.empty }

let rec remove_ctx : type x a mu n b. (x, b) ctx -> (a, mu, n, b) insert -> (x, a) ctx =
 fun ctx i ->
  match (ctx, i) with
  | Snoc (ctx, vars, flds), Later i -> Snoc (remove_ctx ctx i, vars, flds)
  | Snoc (ctx, _, _), Now -> ctx
  | Other (ctx, o), Later i -> Other (remove_ctx ctx i, o)
  | Other (ctx, _), Now -> ctx
  | Emp, _ -> .
  | Lock _, _ -> .

let remove : type x a mu n b. (x, b) t -> (a, mu, n, b) insert -> (x, a) t =
...
let rec extract_ctx : type x a mu n b.
    (x, b) ctx -> (a, mu, n, b) insert -> (x, a) ctx * (x, n) entry =
 fun ctx i ->
  match (ctx, i) with
  | Snoc (ctx, vars, flds), Now -> (ctx, Entry_vars (vars, flds))
  | Other (ctx, o), Now -> (ctx, Entry_other o)
...
let rec permute_ctx : type x a b. (a, b) permute -> (x, b) ctx -> (x, a) ctx =
 fun p ctx ->
  match p with
  | Id -> ctx
  | Insert (p, i) -> (
      let rest, e = extract_ctx ctx i in
      match e with
      | Entry_vars (vars, flds) -> Snoc (permute_ctx p rest, vars, flds)
      | Entry_other o -> Other (permute_ctx p rest, o))
  | Lock p ->
      let (Lock ctx) = ctx in
      Lock (permute_ctx p ctx)

let permute : type x a b. (a, b) permute -> (x, b) t -> (x, a) t =
...
let rec split_ctx : type x a b ab p q r. (x, ab) ctx -> (p, b, q, a, r, ab) Tctx.comp -> (x, a) ctx
    =
 fun ctx ab ->
  match (ctx, ab) with
  | _, Zero -> ctx
  ...
  | Other (ctx, _), Suc (ab, Dim _) -> split_ctx ctx ab
  | Lock ctx, Suc (ab, Lock _) -> split_ctx ctx ab

let split : type x a b ab p q r. (x, ab) t -> (p, b, q, a, r, ab) Tctx.comp -> (x, a) t =
...
let lookup : type x mode a. (x, a) t -> (mode, a) index -> [ `Name of string list | `Other of x ] =
 fun { ctx; used = _ } (Index (v, fa, _, Plus_with_locks (comp, _))) ->
  let rec lookup : type a modality k n an.
      (x, an) ctx ->
      (a, modality, n, an) insert ->
      (k, n) sface ->
      [ `Name of string list | `Other of x ] =
   fun ctx v fa ->
    match (ctx, v) with
    | Snoc (ctx, _, _), Later v -> lookup ctx v fa
    | Other (ctx, _), Later v -> lookup ctx v fa
    | Other (_, o), Now -> `Other o
    | Snoc (_, Variables (_, mn, xs), fields), Now ->
        ...
        `Name (cubevar (NICubeOf.find xs fc) fb)
    | Emp, _ | Lock _, _ -> . in
  lookup (split_ctx ctx comp) v fa

(* Look up an index variable together with a field, to find a name for the combination, if there is one. *)
let lookup_field : type x mode a. (x, a) t -> (mode, a) index -> string -> string list option =
 fun { ctx; used = _ } (Index (v, fa, _, Plus_with_locks (comp, _))) f ->
  let rec lookup : type a modality k n an.
      (x, an) ctx -> (a, modality, n, an) insert -> (k, n) sface -> string list option =
   fun ctx v fa ->
    match (ctx, v) with
    | Snoc (ctx, _, _), Later v -> lookup ctx v fa
    | Other (ctx, _), Later v -> lookup ctx v fa
    (* An "other" entry exposes no fields. *)
    | Other (_, _), Now -> None
    ...
    NICubeOf.map
      {
        map =
          (fun _ name ->
            let name, u = uniquify_nfamof name !used in
            used := u;
            name);
      }
      names in
...
let add_cube : type x m n b.
    n D.t -> (x, b) t -> binder_name -> string * (x, (b, (m, n) dim_entry) snoc) t =
 fun n { ctx; used } name ->
...
(* Add a variable that is displayed not as a name but as the client-supplied payload that "lookup" returns for it. *)
let add_other : type x m n b. (x, b) t -> x -> (x, (b, (m, n) dim_entry) snoc) t =
 fun { ctx; used } o -> { ctx = Other (ctx, o); used }
...
let rec add_match_vars : type x n mode annotations a b ab.
    (x, a) t ->
...
let add_fields : type x b n m.
    n D.t -> (x, b) t -> string list -> ((x, (b, (m, n) dim_entry) snoc) t * string list) option =
...
let add_strings : type x b m n.
    (x, b) t -> (n, string) gvariables -> (n, string) gvariables * (x, (b, (m, n) dim_entry) snoc) t
    =
 fun { ctx; used } (Variables (m, mn, names)) ->
...
let add_full : type x b k mn.
    (x, b) t -> mn variables -> (mn, string) gvariables * (x, (b, (k, mn) dim_entry) snoc) t =
...
let rec add_lock : type x a dom modality mode am.
    (x, a) t -> (a, mode, modality, dom, am) plus_lock -> (x, am) t =
 fun ctx -> function
...
let rec of_ordered_ctx : type x mode a b. (mode, a, b) Ctx.Ordered.t -> (x, b) t = function
  | Emp _ -> empty
  ...
  | Lock (ctx, _) ->
      let { ctx; used } = of_ordered_ctx ctx in
      { ctx = Lock ctx; used }
  (* A weakening entry has no checked variable, so it contributes no name. *)
  | Weaken (ctx, _) -> of_ordered_ctx ctx

let of_ctx : type x mode a b. (mode, a, b) Ctx.t -> (x, b) t = function
...
let degenerate : type x r b kb mode. r D.t -> (r, b, kb, mode) plusmap -> (x, b) t -> (x, kb) t =
 fun r pm { ctx; used } ->
  ...
  let rec go : type c kc m. (r, c, kc, m) plusmap -> (x, c) ctx -> (x, kc) ctx =
   fun pm ctx ->
    match ctx with
    | Emp -> (
        match pm with
    ...
    | Other (ctx, o) -> (
        match pm with
        | Suc (pm, Inject (Plus_dim _), Suc (Zero, Dim _)) -> Other (go pm ctx, o))
    | Lock ctx -> (
        match pm with
...
let unsafe_add : type x k n b.
    (x, b) t ->
    (n, string) gvariables ->
    (string, string) Abwd.t ->
    (x, (b, (k, n) dim_entry) snoc) t =
