...
(* Make a variable name or placeholder unique, assigning an autogenerated name to placeholders. *)
let uniquify_opt : type a.
 ...
 fun f name used ->
  match f name with
  | None, face -> new_name face (Display.variables ()) used
  | Some name, face -> uniquify (name ^ face) used
...
(* Add a single (cube) self-variable for a record type, with its fields exposed as named variables (mirroring Ctx.vis_fields and of_ordered_ctx).  The self-variable itself is anonymous: its fields are exposed so that field-projections of the self read back as field variables.  Returns the field variable names (in order) for use as the field patterns. *)
let add_fields : type b n. n D.t -> b t -> string list -> (b, n) snoc t * string list =
 fun n { ctx; used } field_names ->
  let self_name, _, used = uniquify_opt (fun x -> (x, "")) None used in
  let rec go used = function
    | [] -> (Abwd.empty, [], used)
    | f :: fs ->
        let v, _, used = uniquify f used in
        let fields, vars, used = go used fs in
        (Snoc (fields, (f, v)), v :: vars, used) in
  let fields, var_names, used = go used field_names in
  ( { ctx = Snoc (ctx, Variables (n, D.plus_zero n, NICubeOf.singleton self_name), fields); used },
    var_names )
...
let add_strings : type b n. b t -> (n, string) gvariables -> (n, string) gvariables * (b, n) snoc t
    =
 fun { ctx; used } (Variables (m, mn, names)) ->
  let names, used = uniquify_cube (fun x -> (Some x, "")) names used in
  let vars = Variables (m, mn, names) in
  (vars, { ctx = Snoc (ctx, vars, Abwd.empty); used })
  ...
  let rec go : type n.
   ...
   fun vars used ->
    ...
    | Snoc (vars, (x, sh)) ->
        let x, orig, used =
          match (x, sh) with
          | Some x, `Visible -> (x, `Original, used)
          (* Unnamed variables are always marked as shadowed by used_vars, so the remaining cases are all renamed. *)
          | _, _ ->
