...
struct
  ...
  let unit = Modalcell.of_gen (Modalcell.generate (Modality.id typ) amb)
  let mult = Modalcell.of_gen (Modalcell.generate ambamb amb)

  (* Coreflector data: counit and comultiplication. *)
  let counit = Modalcell.of_gen (Modalcell.generate amb (Modality.id typ))
  let comult = Modalcell.of_gen (Modalcell.generate amb ambamb)
